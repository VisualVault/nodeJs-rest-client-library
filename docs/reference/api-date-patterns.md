...
> Based on confirmed test results from the [WS date-handling investigation](../../research/date-handling/web-services/analysis/overview.md) (148 tests, WS-1 through WS-10).
...
**Why `forminstance/` for all DateTime fields**: The `postForms` endpoint appends Z to datetime values, which triggers a timezone shift when users open the record in Forms (WEBSERVICE-BUG-1). The `forminstance/` endpoint stores in US format without Z — no shift occurs. Empirically verified: `getSaveValue()` stores identical values for `ignoreTZ=true` and `ignoreTZ=false` — the flag affects display only, not DB storage. See [WEBSERVICE-BUG-1](../../research/date-handling/web-services/analysis/ws-bug-1-cross-layer-shift.md).
...
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| WS-1   | Server TZ irrelevant for API writes (H-4)                                                                                                                                                                                                             | [matrix.md WS-1](../../research/date-handling/web-services/matrix.md)                       |
| WS-4   | API Z normalization → Forms TZ-dependent display (CB-8)                                                                                                                                                                                               | [ws-4-batch-run-1.md](../../research/date-handling/web-services/runs/ws-4-batch-run-1.md)   |
| WS-5   | Server converts TZ offsets to UTC correctly (CB-12)                                                                                                                                                                                                   | [ws-5-batch-run-1.md](../../research/date-handling/web-services/runs/ws-5-batch-run-1.md)   |
| WS-5   | DD/MM/YYYY silently stored as null — WEBSERVICE-BUG-2                                                                                                                                                                                                 | [ws-5-batch-run-1.md](../../research/date-handling/web-services/runs/ws-5-batch-run-1.md)   |
| WS-9   | TZ-safe vs unsafe Date patterns (CB-24–CB-28)                                                                                                                                                                                                         | [ws-9-batch-run-1.md](../../research/date-handling/web-services/runs/ws-9-batch-run-1.md)   |
| Cat 10 | Epoch silent null, midnight-crossing                                                                                                                                                                                                                  | [cat10-gaps-run-1.md](../../research/date-handling/web-services/runs/cat10-gaps-run-1.md)   |
| WS-10  | postForms vs forminstance/ serialization format (CB-29)                                                                                                                                                                                               | [ws-10-batch-run-1.md](../../research/date-handling/web-services/runs/ws-10-batch-run-1.md) |
| Cat 16 | Server TZ irrelevant for form save pipeline too — vvdemo (BRT) and vv5dev (PDT) produce identical getForms API responses and identical reload values for all 3 configs (A, C, D). Extends WS-1 finding to the browser form save→server→API-read path. | [matrix.md Cat 16](../../research/date-handling/forms-calendar/matrix.md)                   |
