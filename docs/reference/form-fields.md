...
| #7    | Wrong day stored for UTC+ timezones                     | Date-only fields, all configs (`enableTime=false`)          | High — UTC+ users store previous day on every SetFieldValue call; field display shows the user-typed date, masking the shift until form reload                                                                 |
