...
| `getSaveValue(date, enableTime, ignoreTimezone, useLegacy)` | `function` | Converts a Date object to storage format string. Strips Z suffix. For date-only (`enableTime=false`): extracts the **UTC date** via `toISOString()` → takes date portion → produces `"yyyy-MM-dd"`. This is the mechanism behind Bug #7: IST local midnight = previous UTC day → stores wrong date (DB dump: DateTest-000084 Field7 = `2026-03-14` for IST user who picked March 15). For DateTime: produces `"yyyy-MM-ddTHH:mm:ss"`. **Empirically verified**: produces identical output for `ignoreTZ=true` and `ignoreTZ=false` given the same Date object — the flag affects display only, not storage (DB dump 2026-04-06). |
