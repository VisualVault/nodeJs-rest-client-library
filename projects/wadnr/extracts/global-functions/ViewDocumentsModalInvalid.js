/**
 * VV.Form.Global.ViewDocumentsModalInvalid
 * Parameters: 1
 * Extracted: 2026-04-10
 */
function (documentName) {
/*
  Script Name:   ViewDocumentsModalInvalid
  Customer:      WADNR
  Purpose:       The purpose of this process is to invalidate, unrelate, and update document index for a document and its row in the View Documents Modal. Is a component of VV.Form.Global.ViewDocumentsModal. 
...
*/
const websvcName = "LibViewDocumentsModalInvalid";
const { documentID, documentGUID, documentMetadataHTML } =
  getDocumentDataFromMap(documentName);
...
let jqMainContent = $(
  `<div style="display:flex;align-items:center;flex-flow:column;">
    <p style="margin-bottom:1rem;">Do you want to mark the following document as invalid? If you proceed, the document will be unassociated from this record but will still be retained in the system.?</p>
...
Swal.fire({
  icon: "question",
  title: "Confirm Mark Document Invalid",
  ...
  if (result.dismiss === Swal.DismissReason.cancel) {
    VV.Form.Global.ViewDocumentsModal(
      VV.Form.Global.ViewDocumentsModalLastProperties.options
    );
...
function getDocumentDataFromMap(documentName) {
  ...
  try {
    documentData =
      VV.Form.Global.ViewDocumentsModalLastProperties.documentDataMap.get(
        documentName
      );
  } catch (error) {
    documentData = null;
  ...
  if (documentData == null || Object.keys(documentData).length < 1) {
    ...
    if (documentData.documentMetadataHTML == null) {
      errorLog.push(
        `A documentMetadataHTML was not found in document data map.`
      );
    }
  }
...
function displayErrorModalAndThrow(messageData, title = "Error") {
  VV.Form.Global.DisplayModal({
    Title: title,
    Icon: "error",
    HtmlContent: Array.isArray(messageData)
      ? messageData.join("<br>")
      : messageData,
  });
  throw new Error(
...
function callViewDocumentsModalAccept() {
  let CallServerSide = function () {
    ...
    FormInfo = {};
    FormInfo.name = "documentGUID";
    FormInfo.value = documentGUID;
    formData.push(FormInfo);
    ...
    if (typeof resp.status != "undefined") {
      ...
      if (resp.data[0] != "undefined") {
        if (resp.data[0] == "Success") {
          ...
          Swal.fire({
            icon: "success",
            title: "Document Invalidated",
            html: "The document has been successfully unassociated from this record and remains stored in the system.",
            showConfirmButton: true,
            confirmButtonText: "OK",
          }).then((result) => {
            if (result.isConfirmed) {
              // Call the ViewDocuments function when the OK button is clicked
              ViewDocuments();
            }
          });
...
function ViewDocuments() {
  if (
    VV.Form.Global.ViewDocumentsModalLastProperties.documentDataMap.size > 1
  ) {
