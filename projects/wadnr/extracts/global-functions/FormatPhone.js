...
if (s2.length < 10) {   //Have not fully keyed in the phone number so just return what they have keyed in.
    return phoneNumber;
}
else if (s2.length > 15) {  //Have too many numbers for a US number, return phone number.
    return phoneNumber;
}
else if (s2.length == 10) {  //Phone number is a number without extension.
    var m = s2.match(/^(\d{3})(\d{3})(\d{4})$/);
    return (!m) ? null : "(" + m[1] + ") " + m[2] + "-" + m[3];
}
else {      //Phone number has an extension, format it into the 4 groups.
