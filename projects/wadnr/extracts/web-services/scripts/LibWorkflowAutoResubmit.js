/**
 * LibWorkflowAutoResubmit
 * Category: Workflow
 * Modified: 2026-03-19T12:59:43.023Z by mauro.rapuano@visualvault.com
 * Script ID: Script Id: 8341c97e-86af-f011-82f3-8a51ceac8dc2
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
      Script Name:      LibWorkflowAutoResubmit
      Customer:         WADNR
        Purpose:        Automates resubmissions for fee-based applications (FPAN, LTA Step 1, Renewal) once they are Paid. 
                        It executes the same business logic as the manual Resubmit button—including ARP update, 
                        field updates, status transitions, status history creation, and notifications.
      Parameters:     
      ...
      Psuedo code: 
                        ...
                        4. Do resubmit-equivalence process
                            4.1 Call LibUpdateApplicationReviewPage
                            4.2 Create Status History
                            4.3 Update form's fields and send email notifications
                        5. Call library that collect and relate Forms and Docs to an ARP
                        6. Build data response object
  
      Date of Dev:      10/21/2025
      Last Rev Date:    03/18/2026
      ...
      Revision Notes:
                        ...
                        01/09/2026 - Fernando Chamorro: Logic is implemented that associates form creation/update with the 
                                                        ARP and creates its respective AssociatedDocumentRelation record
                        01/16/2026 - Fernando Chamorro: Removing Associated Document Relation logic
                        01/20/2026 - Fernando Chamorro: Call library that collect and relate Forms and Docs to an ARP
                        03/18/2026 - Mauro Rapuano:     Updated code to use WA Timezone 
      */

    logger.info('Start of the process LibWorkflowAutoResubmit at ' + Date());
    ...
    const libApplicationReviewCollectRelatedDocs = 'LibApplicationReviewCollectRelatedDocs';

    const notResubmittedStatus = ['Needs Resubmission', 'Need Resubmission'];
    ...
    async function getFormData(formId, formTemplate) {
        const shortDescription = `Get form ${formTemplate}`;
        const getFormsParams = {
            q: `[instanceName] eq '${formId}'`,
            //expand: true, // true to get all the form's fields
            fields: 'revisionId, instanceName, Fees Paid, AutoResubmitNeeded, Status, FPAN Number, Region, Project Name, Long Term Application',
        };
    ...
    async function callLibApplicationReviewCollectRelatedDocs(formId, fpaNumber) {
        const shortDescription = `Run Web Service: ${libApplicationReviewCollectRelatedDocs}`;
        ...
        return vvClient.scripts
            .runWebService(libApplicationReviewCollectRelatedDocs, webServiceParams)
            .then((res) => checkMetaAndStatus(res, shortDescription))
            .then((res) => checkDataPropertyExists(res, shortDescription))
            .then((res) => checkDataIsNotEmpty(res, shortDescription))
            .then((res) => res.data);
    ...
    async function getARPDataByFPANNumber(fpanNumber) {
        ...
        const getFormsParams = {
            ...
            //expand: true, // true to get all the form's fields
            fields: 'revisionId, instanceName, fpaN Number, fpaN Date of Receipt, ltA Date of Receipt, fpaN Renewal Date of Receipt',
        };
    ...
    async function getARPDataByRelatedRecordID(formId) {
        ...
        const getFormsParams = {
            ...
            //expand: true, // true to get all the form's fields
            fields: 'revisionId, instanceName, fpaN Number, Application Type',
        };
    ...
    async function getARPDataByRenewalID(formId) {
        const shortDescription = `Get Application Review Page for ${formId}`;
        const getFormsParams = {
            q: `[Renewal ID] eq '${formId}'`,
            //expand: true, // true to get all the form's fields
            fields: 'revisionId, instanceName, fpaN Number, Application Type',
        };
    ...
    try {
        ...
        // 3. Check preconditions (Fees Paid = True, AutoResubmitNeeded = True, Not already resubmitted)
        // Get form record
        const formTemplate = getFormName(formType);
        const applicationRecord = await getFormData(formId, formTemplate);
        ...
        const oldStatus = applicationRecord[0]['status'];
        const isResubmitted = !notResubmittedStatus.includes(applicationRecord[0]['status']);

        if (!isResubmitted) {
            if (applicationRecord[0]['fees Paid'] !== 'True') {
                throw new Error('Fees Paid not set');
            }
            if (applicationRecord[0]['autoResubmitNeeded'] !== 'True') {
                throw new Error('AutoResubmitNeeded not set');
            }
            ...
            const regionCode = getRegionCode(applicationRecord[0]['region']);

            if (formType === 'renewal') {
                arpPreviousData = await getARPDataByRenewalID(applicationRecord[0]['instanceName']);
            } else {
                arpPreviousData = await getARPDataByRelatedRecordID(applicationRecord[0]['instanceName']);
            }
            ...
            arpData = await callLibUpdateApplicationReviewPage(
                formId,
                regionCode,
                applicationRecord[0]['fpaN Number'],
                appType
            );

            if (!arpData || !arpData[0] || arpData[0] != 'Success') {
                throw new Error('Error updating ARP');
            }
            ...
            // 4.3 Update form's fields and send email notifications
            await updateFormRecord(formTemplateName, applicationRecord[0]['revisionId'], {
                Status: 'Revision Received',
                'Resubmitted Date': dayjs(new Date().toLocaleString())
                    .tz('America/Los_Angeles')
                    .format('MM/DD/YYYY HH:mm:ss'),
                UpdateSubformsToSubmitted: 'True',
            });
        } else {
            arpData = await getARPDataByFPANNumber(applicationRecord[0]['fpaN Number']);

            if (arpData.length === 0) {
                throw new Error(`Application Review Page not found for ${applicationRecord[0]['fpaN Number']}`);
            } else {
                arpData = arpData[0];
            }
        ...
        // 5. Call library that collect and relate Forms and Docs to an ARP
        const arpCollectRelatedDocs = await callLibApplicationReviewCollectRelatedDocs(formId, fpaNumber);

        if (arpCollectRelatedDocs[0] === 'Success') {
            logger.info('Documentation Related successfully');
        }
        ...
        const dataObj = {
            ...
            Transitions: [oldStatus, 'Revision Received'],
            ArpId: arpID,
