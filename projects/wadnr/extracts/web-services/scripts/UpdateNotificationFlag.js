 ...
 * Category: Workflow
 * Modified: 2025-04-21T22:46:11.29Z by joaquin.godoy@visualvault.com
 * Script ID: Script Id: 4aa1cf79-ef1a-f011-82d5-cb8a21848379
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
        Script Name:    UpdateNotificationFlagFromReviewPage
        Customer:       WADNR
        Purpose:        This script is used to update the notification flag on the FPAN, LTA and Aerial Chemical
                        forms when the status is changed on the Application Review Page.
        ...
        Return Object:  outputCollection[0]: Status
                        outputCollection[1]: Short description message
                        outputCollection[2]: Data

        Pseudo code:    1 - Get the value of the Related Record ID
                        2 - Check which form it is and get the form data.
                        3 - Update the notification flag on that specific form.
                        4 - Build the success/error response array.
                        5 - Send the response to the client.

        Date of Dev:    04/16/2025

        Last Rev Date:  04/16/2025

        Revision Notes:
                        04/16/2025 - Joaquin Godoy:  First Setup of the script
    */

    const WEB_SERVICE_NAME = 'UpdateNotificationFlagFromReviewPage';
    logger.info(`Start of the process ${WEB_SERVICE_NAME} at ${Date()}`);
    ...
    const NOTIFICATION_FLAG_FIELD_NAME = 'forest Practice Notification Needed';
    ...
    function getFormRecords(filterValue, templateName) {
        ...
        const getFormsParams = {
            q: `[instanceName] eq '${filterValue}'`,
            fields: NOTIFICATION_FLAG_FIELD_NAME,
        };
    ...
    function updateRecord(formTemplateName, recordGUID) {
        ...
        const fieldValuesToUpdate = {
            ['Forest Practice Notification Needed']: true,
        };
    ...
    try {
        // 1 - Get the value of the Related Record ID
        ...
        const data = await getFormRecords(relatedRecordID, formTemplateName);

        // 3 - Update the notification flag on that specific form.
        const revisionId = data[0].revisionId;
        const notificationFlag = data[0][NOTIFICATION_FLAG_FIELD_NAME];
        if (notificationFlag === undefined) {
            throw new Error(
                `The field '${NOTIFICATION_FLAG_FIELD_NAME}' was not found in the form ${formTemplateName}.`
            );
        }

        await updateRecord(formTemplateName, revisionId);

        // 4 - Build the success response array.
        outputCollection[0] = 'Success'; // Don´t change this
        outputCollection[1] = 'Notification flag changed on the related record.';
        outputCollection[2] = {
            newNotificationFlagValue: true,
        };
    } catch (error) {
        // 4 - Build the error response array.
