/**
 * ICNRelateToARPorWTMRP
 * Category: Workflow
 * Modified: 2025-10-06T13:07:20.74Z by lucas.herrera@visualvault.com
 * Script ID: Script Id: e80ae26e-b194-f011-82f6-c569e28d17d7
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
    Script Name:    ICNRelateToARPorWTMRP
    Customer:       WADNR
    Purpose:        The purpose of this script is to update or create Associated Document Relations between ICN and ARP and/or WTMF records based on the IDs provided. Forms will be related each other.
    ...
    Parameters:     The following represent variables passed into the function:
                    ...
                    - FPAN or WTMF Numbers:  This is the list of FPAN and/or WTMF Numbers so we can find the records
                                             to associate.
    Return Object:
    ...
    Pseudo code: 
                    1° Get, if exist, the related ARP and/or WTMF records for current ICN Record.
                    2° Call the library LibApplicationReviewCollectRelatedDocs that update or create Document Associated Relation
                    3° Related the current record with the related ARP and/or WTMF records
                    4° Relate the current form with ARP and/or WTMF records
                    5° Return the output collection
 
    Date of Dev:    09/16/2025
    Last Rev Date:  09/16/2025
 
    Revision Notes:
                    09/16/2025 - Federico Cuelho:  First Setup of the script.
    */

    logger.info(`Start of the process ICNRelateToARPorWTMRP at ${Date()}`);
    ...
    async function relateForms(FPANOrWTMFNumberString, formId) {
        /*
        Function to relate the current form with ARP and/or WTMF records based on the provided numbers
        Parameters:
            FPANOrWTMFNumberString: A comma-separated string of FPAN and/or WTMF numbers
            formId: The Doc ID of the current form
        */

        // Split the string by commas and trim whitespace
        const idNumbers = FPANOrWTMFNumberString.split(',').map((num) => num.trim());

        for (const id of idNumbers) {
            try {
                const type = classifyNumber(id);
                const templateName = type === 'FPAN' ? AppReviewPageTemplateName : WTMReviewPageTemplateName;
                const parentRecordGUID = await getRecordGUID(templateName, id);
    ...
    try {
        // 1° Get parameters
        const icnFormId = getFieldValueByName('Form ID');
        const FPANOrWTMFNumberString = getFieldValueByName('FPAN or WTMF Numbers');

        // 2° Check is the required parameters are present
        if (!icnFormId || !FPANOrWTMFNumberString) {
        ...
        const webServiceParams = [
            ...
            {
                name: 'FPAN or WTMF Numbers',
                value: FPANOrWTMFNumberString,
            },
        ];
        ...
        // 4° Relate the current form with ARP and/or WTMF records
        await relateForms(FPANOrWTMFNumberString, icnFormId);
