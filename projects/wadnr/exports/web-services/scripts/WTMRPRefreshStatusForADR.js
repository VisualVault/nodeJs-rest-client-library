/**
 * WTMRPRefreshStatusForADR
 * Category: Form
 * Modified: 2026-01-08T20:40:40.58Z by sebastian.rolando@visualvault.com
 * Script ID: Script Id: 6724ea36-d2ec-f011-8301-c8928c0a9d8e
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
    Script Name:    WTMRPRefreshStatusForADR 
    Customer:       WADNR
    Purpose:        Update Document Status field for Associated Document Relation records related to the WTMRP, based on the main application Status
    Preconditions:  -
    ...
    Last Rev Date:  01/08/2026
 
    Revision Notes:
                    01/08/2026 - Sebastian Rolando:  First Setup of the script
    */

    logger.info(`Start of the process WTMRPRefreshStatusForADR at ${Date()}`);
    ...
    try {
        ...
        const WTMRPID = getFieldValueByName('Form ID');
        const mainAppID = getFieldValueByName('Related Record ID');

        // 2° CHECK IF THE REQUIRED PARAMETERS ARE PRESENT

        if (!WTMRPID || !mainAppID) {
        ...
        // 5° Get Associated Document Relation records related to the ARP
        const associatedDocumentRelationRecordsList = await getAssociatedDocumentRelationRecords(WTMRPID);
