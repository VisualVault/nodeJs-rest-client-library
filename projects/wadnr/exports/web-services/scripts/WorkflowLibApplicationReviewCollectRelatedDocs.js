/**
 * WorkflowLibApplicationReviewCollectRelatedDocs
 * Category: Workflow
 * Modified: 2026-03-24T21:36:16.043Z by john.sevilla@visualvault.com
 * Script ID: Script Id: 3ad26a50-03b1-f011-82fc-9fab3cd69972
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
      Script Name:    WorkflowLibApplicationReviewCollectRelatedDocs
      Customer:       WADNR
      ...
      Revision Notes:
                      ...
                      03/24/2026 - John Sevilla: Update classifyNumber to account for legacy FPA numbers
      */

    logger.info('Start of the process WorkflowLibApplicationReviewCollectRelatedDocs at ' + Date());
    ...
    const printOrderObjDocumentsWTMF = {
        ...
        'Water Typing Map': 2,
        'Water Protocol Survey': 3,
        'Water Type Modification Form (Processed)': 4,
        'Other - WTMF': 5,
        'Informal Conference Notification': 7,
        Other: 8,
    };

    let printOrderCounter = 42;

    let printOrderCounterWTMF = 9;
    /* -------------------------------------------------------------------------- */
    /*                           Script Variables                                 */
    /* -------------------------------------------------------------------------- */
    ...
    function classifyNumber(number) {
        ...
        */
        ...
        if (number.includes('-FPA-') || /^\d+$/.test(number)) {
            // If it matches either the new fpOnline FPA number format OR the legacy FPA number format (only digits)
            return 'FPAN';
        } else if (number.includes('-WTM-')) {
            // If it matches the WTMF number format
            return 'WTMF';
        } else {
    ...
    async function getAssociatedDocumentRelationRecords(formID) {
        ...
        const getFormsParams = {
            ...
            // expand: true,
            fields: 'related Record ID, document or Form, arP ID or WTM RP ID',
        };
    ...
    async function getApplicationRecordsAndRelatedLists(ApplicationID) {
      ...
      */
        ...
        // Filter CONTACT-INFORMATION, CONTACT-INFORMATION-RELATION and Business records. Also filter APPLICATION-REVIEW and WTMRP to prevent to relate cross-referenced data
    ...
    try {
            ...
            for (const arpRecord of arpRecords) {
                ...
                if (arpFormID) {
                    ...
                    await processApplicationRelatedRecords(
                        applicationRelatedRecords,
                        docsResults,
                        applicationRelatedFormsList, // object-IDs
                        applicationRelatedDocsList, // object-IDs
        ...
        returnObject = {
            ...
            Success: 'Documentation Related successfully',
        };
    } catch (err) {
