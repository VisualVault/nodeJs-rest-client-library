/**
 * LibAssignPermissionsToAFolder
 * Category: Workflow
 * Modified: 2025-12-03T15:49:23.553Z by fernando.chamorro@visualvault.com
 * Script ID: Script Id: 5d758e87-5ed0-f011-8306-f1dc58f01122
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
      Script Name:      LibAssignPermissionsToAFolder
      Customer:         WADNR
      Purpose:          Add folder permissions in the document library.
      ...
      Return Object:
                        ...
                        outputCollection[2]: [
                            "Success",
                            "Permission 'Viewer' applied to Group 'Proponent' on folder '/FPA/YYYY/11-11/NW-FPA-11-11/Applications'.",
                            {
                                folderGuid: "abc11-abc1-abc111-abc1-abc11",
                                newEntity: "Proponent",
                                newEntityType: "Group",
      ...
      Pseudo code:
                        ...
                        3. Assign permissions to the specified folder
                        4. Build the success response array

      Date of Dev:      12/03/2025
      Last Rev Date:    12/03/2025

      Revision Notes:
                        12/03/2025 - Fernando Chamorro:  First Setup of the script

      */

    logger.info(`Start of the process LibAssignPermissionsToAFolder at ${Date()}`);
    /* -------------------------------------------------------------------------- */
    ...
    try {
        ...
        const applyToSubfolders = getFieldValueByName('Apply to subfolders');

        // 2. Check if the required parameters are present
        if (!newEntity || !newEntityType || !permissionType || !folderPath || !applyToSubfolders) {
        ...
        if (isNullEmptyUndefined(addFolderPermissions)) {
            throw new Error(`Error while setting new folder permissions - ${newEntity} not found`);
        }

        // 4. Build the success response array
        outputCollection[0] = 'Success'; // Don´t change this
        outputCollection[1] = addFolderPermissions[1];
        outputCollection[2] = addFolderPermissions;
    } catch (error) {
