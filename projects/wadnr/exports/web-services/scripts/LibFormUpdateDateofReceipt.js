/**
 * LibFormUpdateDateofReceipt
 * Category: Workflow
 * Modified: 2026-01-29T18:44:28.49Z by lucas.herrera@visualvault.com
 * Script ID: Script Id: 33ad6f89-3efd-f011-8311-c41c113564c7
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
    Script Name:    LibFormUpdateDateofReceipt
    Customer:       WADNR
    Purpose:        Locates an existing form in the fpOnline System by its unique Form ID and updates its "Date of Receipt"
                    field with a new value.
    TA Reference:   10.5.5.3
    Preconditions:  FormTemplatePrefixListQueryName
    Parameters:
                    Form ID (String, Required)
                    Date of Receipt (String, Required) - The new date to assign to the form record
    Return Object:
    ...
    Pseudo code:
                    ...
                    4.  Update the form record's Date of Receipt field

    Date of Dev:    01/29/2026
    Last Rev Date:  01/29/2026
    Revision Notes:
                    01/29/2026 - Lucas Herrera: First setup of the script
    */

    logger.info(`Start of the process LibFormUpdateDateofReceipt at ${Date()}`);
    ...
    */
    ...
    try {
        ...
        const dateOfReceipt = getFieldValueByName('Date of Receipt');

        // CHECK IF THE REQUIRED PARAMETERS ARE PRESENT
        if (!formID || !dateOfReceipt) {
        ...
        // Step #4
        const previousDateOfReceipt = formRecordToUpdate['dateOfReceipt'];
        const formRecordRevisionId = formRecordToUpdate.revisionId;
        let fieldToUpdate = '';

        switch (formTemplateName) {
            case 'Forest Practices Aerial Chemical Application':
                fieldToUpdate = 'Received Date';
                break;
            default:
                fieldToUpdate = 'Date of Receipt';
        }

        const updatedFormRecord = await updateFormRecord(
            { [fieldToUpdate]: dateOfReceipt },
            formTemplateName,
            formRecordRevisionId,
            `Update form record ${formRecordRevisionId}`
        );

        outputCollection[0] = 'Success'; // Don´t change this
        outputCollection[1] = `Form Date of Receipt updated from ${previousDateOfReceipt} to ${dateOfReceipt}`;
        outputCollection[2] = {
            templateName: formTemplateName,
            formGUID: updatedFormRecord.revisionId,
            [fieldToUpdate]: dateOfReceipt,
        };
    } catch (error) {
