 ...
 * postForms
 * Category: Form
 * Modified: 2021-06-29T15:30:30.037Z by emanuel.jofre@onetree.com
 * Script ID: Script Id: 00c943dc-eed8-eb11-8204-e3982f57380d
...
module.exports.main = async function (ffCollection, vvClient, response) {
  ...
  const formTemplateId = "postForm";

  if (!targetFields.name) {
    returnObj = ["Error", "Name undefined"];
  } else {
    try {
      const res = await vvClient.forms.postForms(
        null,
        targetFields,
        formTemplateId
      );
      if (res.meta.status === 201) {
        returnObj = ["Success", "New post created ok."];
      } else {
        returnObj = ["Error", "New post creation failed."];
      }
    } catch (error) {
