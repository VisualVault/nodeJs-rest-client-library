...
**Key outcome**: FORM-BUG-5 confirmed; -3h drift per trip established
...
**Key Finding**: Both popup (Scenario 1) and typed input (Scenario 2) produce **identical stored values**. FORM-BUG-2 (inconsistent handlers) was **NOT reproduced** with this config (`useLegacy=false`, `enableTime=true`, `ignoreTimezone=true`).

### Test 1.3: FORM-BUG-5 — SetFieldValue/GetFieldValue Round-Trip
...
**CONFIRMED**: FORM-BUG-5 causes a **date shift on every round-trip**. The fake `[Z]` from `getCalendarFieldValue()` makes the value LOOK like UTC but it's actually local time. When `SetFieldValue` receives it back, JS interprets the Z as real UTC, shifting the date by the timezone offset.
...
## Session 1, Group 2: Multi-Config Baseline and FORM-BUG-5 Deep-Dive — DateTest Form (BRT)
...
**Purpose**: Full config matrix baseline, FORM-BUG-5 mechanics, popup vs typed parity, server reload, DB evidence
**Key outcomes**: FORM-BUG-2 not reproduced; DB stores mixed UTC/local; typed = popup with useLegacy=false

### Test Form Configuration

**8 fields with different configurations** (Field10 added during session):

| Field   | enableInitialValue | Mode            | enableTime | ignoreTZ | useLegacy | Config ID |
| ------- | :----------------: | --------------- | :--------: | :------: | :-------: | :-------: |
| Field1  |         ✓          | Current Date    |    OFF     |   OFF    |    OFF    |     A     |
| Field2  |         ✓          | Preset 3/1/2026 |    OFF     |   OFF    |    OFF    |     A     |
| Field3  |         ✓          | Current Date    |    OFF     |   OFF    |    OFF    |     A     |
| Field4  |         ✓          | Preset 3/1/2026 |    OFF     |   OFF    |    OFF    |     A     |
| Field5  |        OFF         | —               |   **ON**   |  **ON**  |    OFF    |     D     |
| Field6  |        OFF         | —               |   **ON**   |   OFF    |    OFF    |     C     |
| Field7  |        OFF         | —               |    OFF     |   OFF    |    OFF    |     A     |
| Field10 |        OFF         | —               |    OFF     |  **ON**  |    OFF    |     B     |

_Field1-4 have initial values (used for Scenario 5/6 testing). Field5-7,10 are empty (used for popup/typed/API testing)._
_Field8/9 do not exist — naming jumped from 7 to 10._

### Test 2.1: Baseline — Initial Values on Form Load (Scenarios 5 & 6)

**Observation**: Field1-4 store actual **Date objects** (not strings) in the partition.

| Field                 | Stored Type | ISO Value                  | Display              |
| --------------------- | ----------- | -------------------------- | -------------------- |
| Field1 (Current Date) | Date object | `2026-03-27T20:02:51.472Z` | `03/27/2026` ✓       |
| Field2 (Preset 3/1)   | Date object | `2026-03-01T03:00:00.000Z` | `03/01/2026` ✓       |
| Field3 (Current Date) | Date object | `2026-03-27T20:02:51.475Z` | `03/27/2026` ✓       |
| Field4 (Preset 3/1)   | Date object | `2026-03-01T03:00:00.000Z` | `03/01/2026` ✓       |
| Field5 (empty)        | string      | `""`                       | `MM/dd/yyyy hh:mm a` |
| Field6 (empty)        | string      | `""`                       | `MM/dd/yyyy hh:mm a` |
| Field7 (empty)        | string      | `""`                       | `MM/dd/yyyy`         |
...
- **GetFieldValue for date-only fields (Field1-4)**: Returns the Date object's `.toString()` — e.g., `"Fri Mar 27 2026 17:02:51 GMT-0300 (Brasilia Standard Time)"`. This is neither ISO format nor the raw stored value. Developers using this value would get an unpredictable format.
...
### Test 2.2: Scenario 1 — Calendar Popup (Field5, 6, 7)
...
| ---------- | ----------------- | ----------------------- | ---------------------------- | :---: |
| **Field5** | eTime=✓ iTz=**✓** | `"2026-03-15T00:00:00"` | `"2026-03-15T00:00:00.000Z"` |  NO   |
| **Field6** | eTime=✓ iTz=**✗** | `"2026-03-15T00:00:00"` | `"2026-03-15T03:00:00.000Z"` |  NO   |
| **Field7** | eTime=✗ iTz=✗     | `"2026-03-15"`          | `"2026-03-15"`               |  NO   |

**Critical Finding — GetFieldValue differs by `ignoreTimezone` setting**:

- **Field5 (ignoreTZ=true)**: `getCalendarFieldValue()` adds a **fake literal `[Z]`** via `moment(value).format("....[Z]")`.
  Returns `"2026-03-15T00:00:00.000Z"` — LOOKS like UTC midnight but is actually **local midnight**.

- **Field6 (ignoreTZ=false)**: `getCalendarFieldValue()` does `new Date(value).toISOString()`.
  Returns `"2026-03-15T03:00:00.000Z"` — **correct UTC conversion** of local midnight in UTC-3.

- **Field7 (date-only)**: Returns raw value `"2026-03-15"` — clean, no issues.

### Test 2.3: FORM-BUG-5 — Round-Trip Comparison Across Configs
...
| ---------- | --------- | --------------------- | -------------------------- | --------------------- | ----------------------- | :----------: |
| **Field5** | iTz=**✓** | `2026-03-15T00:00:00` | `2026-03-15T00:00:00.000Z` | `2026-03-14T21:00:00` | **03/14/2026 09:00 PM** | **YES -3h**  |
| **Field6** | iTz=**✗** | `2026-03-15T00:00:00` | `2026-03-15T03:00:00.000Z` | `2026-03-15T00:00:00` | 03/15/2026 12:00 AM     |     NO ✓     |
| **Field7** | date-only | `2026-03-15`          | `2026-03-15`               | `2026-03-15`          | 03/15/2026              |     NO ✓     |

**Root Cause Analysis**:

- **Field5 SHIFTS** because `ignoreTimezone=true` adds fake `[Z]` to local time.
...
- **Field6 is STABLE** because `ignoreTimezone=false` does a proper UTC conversion.
...
- **Field7 is STABLE** because date-only format `"2026-03-15"` has no time or timezone to misinterpret.
...
#### FORM-BUG-5 impact across timezones:
...
**Critical insight**: FORM-BUG-5 shift direction depends on the timezone:
...
| ---------- | ------------- | ----------- | ------------------------------------- | ---------------- | ------------------ |
| **Field5** | eTime+iTz     | `T00:00:00` | `T00:00:00.000Z` (fake Z)             | **`T05:30:00`**  | **+5:30h FORWARD** |
| **Field6** | eTime, no iTz | `T00:00:00` | `T18:30:00.000Z` (real UTC: prev day) | `T00:00:00`      | NONE ✓             |

**Visual**: Field5 display changed from `03/15/2026 12:00 AM` to **`03/15/2026 05:30 AM`** after one round-trip.
...
**Action**: Typed `03/15/2026` (+ `12:00 AM` for DateTime fields) into Field5, 6, 7, and 10 via keyboard.

| Field   | Config        | Typed Raw               | Popup Raw (Test 2.1)    | Match? |
| ------- | ------------- | ----------------------- | ----------------------- | :----: |
| Field5  | eTime=✓ iTz=✓ | `"2026-03-15T00:00:00"` | `"2026-03-15T00:00:00"` | YES ✓  |
| Field6  | eTime=✓ iTz=✗ | `"2026-03-15T00:00:00"` | `"2026-03-15T00:00:00"` | YES ✓  |
| Field7  | eTime=✗ iTz=✗ | `"2026-03-15"`          | `"2026-03-15"`          | YES ✓  |
| Field10 | eTime=✗ iTz=✓ | `"2026-03-15"`          | `"2026-03-15"`          | YES ✓  |

**Conclusion**: FORM-BUG-2 (inconsistent handlers) is **NOT reproduced** with `useLegacy=false`. Both `calChangeSetValue()` (popup) and `calChange()` (typed input) produce identical stored values across ALL tested configurations. The inconsistency described in the analysis may only exist with `useLegacy=true`.

### Test 2.7: Field10 — Date-only + ignoreTimezone=true (New Field)
...
### Test 2.8: Multiple Sequential Round-Trips on Field5
...
| ------ | ---------------------- | --------------------------- | ----------------------- | --------------------- | :----------: |
| Field1 | date-only, CurrentDate | Date `2026-03-27T20:02:51Z` | `"03/27/2026 20:02:51"` | `03/27/2026`          |     NO ✓     |
| Field2 | date-only, Preset 3/1  | Date `2026-03-01T03:00:00Z` | `"03/01/2026 03:00:00"` | `03/01/2026`          |     NO ✓     |
| Field5 | eTime=✓ iTz=✓          | `"2026-03-15T00:00:00"`     | `"2026-03-15T00:00:00"` | `03/15/2026 12:00 AM` |     NO ✓     |
| Field6 | eTime=✓ iTz=✗          | `"2026-03-15T00:00:00"`     | `"2026-03-15T00:00:00"` | `03/15/2026 12:00 AM` |     NO ✓     |
| Field7 | date-only              | `"2026-03-15"`              | `"2026-03-15"`          | `03/15/2026`          |     NO ✓     |
...
2. **Format change for date-only initial value fields** — Field1-4 were originally stored as Date objects but the server returned them in `"MM/dd/yyyy HH:mm:ss"` format. This format change is transparent to the user but represents a different internal representation.

3. **Field2 (Preset 3/1/2026)** — Server stores `"03/01/2026 03:00:00"` (UTC time from local midnight). Despite the time being 03:00 (which was from the UTC-3 conversion), the display is still correct because `initCalendarValueV2()` with `enableTime=false` uses `.startOf("day")`.
...
5. **`initCalendarValueV2()` hardcoded `enableTime=true`** (FORM-BUG-3 from analysis): For the server/database path, `enableTime` is hardcoded to `true`. This means date-only fields loaded from the server are processed as DateTime fields. We didn't observe a visible shift, but this could cause subtle issues when combined with other bugs.
...
Field1: 3/27/2026 8:02:51 PM
Field2: 3/1/2026 3:00:00 AM
Field3: 3/27/2026 8:02:51 PM
Field4: 3/1/2026 3:00:00 AM
Field5: 3/15/2026 12:00:00 AM
Field6: 3/15/2026 12:00:00 AM
Field7: 3/15/2026 12:00:00 AM
```
...
| ------ | ------------------------------------- | ------------------------ | --------------------- |
| Field1 | `new Date().toISOString()` → Date obj | `8:02:51 PM` (20:02 UTC) | **UTC**               |
| Field2 | `parseDateString(preset)` → Date obj  | `3:00:00 AM` (03:00 UTC) | **UTC**               |
| Field5 | popup → `getSaveValue()` strips Z     | `12:00:00 AM`            | **Local (BRT)**       |
| Field6 | popup → `getSaveValue()` strips Z     | `12:00:00 AM`            | **Local (BRT)**       |
| Field7 | popup → date-only string              | `12:00:00 AM`            | **Local (BRT)**       |

**CRITICAL FINDING**: The database contains a **mix of UTC and local times** depending on the code path:

1. **Initial value fields (Field1-4)**: Stored in **UTC** because they go through `new Date()` or `parseDateString()` which produces Date objects, and those get serialized via `toISOString()`.
    - Field1: User was at 17:02 local, but DB has 20:02 (UTC). **3-hour discrepancy**.
    - Field2: Preset "March 1" midnight local → DB has 03:00 AM (UTC).

2. **User-input fields (Field5-7)**: Stored in **local time** because `getSaveValue()` strips the Z suffix and formats without timezone info.
    - Field5-6: March 15 midnight local → DB has 12:00 AM. Correct local representation.
    - Field7: Date-only → stored as midnight.
...
- The same "March 1" date is stored as 3:00 AM in Field2 but would be 12:00 AM in a user-input field
...
**Date**: 2026-03-30 | **TZ**: America/Sao_Paulo (UTC-3) | **Form**: DateTest-000004 — Field5 (D), Field6 (C), Field7 (A)
**Purpose**: Exhaustive SetFieldValue input format matrix + edge cases
**Key outcomes**: FORM-BUG-6 discovered (empty field → truthy "Invalid Date"); bug is in GFV output, not SFV input; year/leap boundaries crossed in 1 trip

### Test 3.1: SetFieldValue Input Formats — Config D (BRT)

All formats tested by calling `VV.Form.SetFieldValue('Field5', value)` then reading raw and GFV.
...
| Test ID       | Input                                            | Raw Stored     | GFV Return     | Notes                                                                                      |
| ------------- | ------------------------------------------------ | -------------- | -------------- | ------------------------------------------------------------------------------------------ |
| 7-A-dateOnly  | `"2026-03-15"` (BRT)                             | `"2026-03-15"` | `"2026-03-15"` | Clean pass in BRT. **In IST → stores `"2026-03-14"` (see FORM-BUG-7)**                     |
...
**Key finding — BRT vs IST divergence (FORM-BUG-7):** Date-only `SetFieldValue` in BRT stores correctly, but in IST (UTC+5:30) every format stores **one day early** (strings) or **two days early** (Date objects).
...
All on Config D (Field5), BRT timezone.
...
#### FORM-BUG-6 Discovery — Empty Field Returns Truthy "Invalid Date"

**FORM-BUG-6 — GetFieldValue returns `"Invalid Date"` for empty DateTime+ignoreTZ fields**
...
**Purpose**: Confirm FORM-BUG-7 live in UTC+; IST drift rate for FORM-BUG-5; cross-TZ load behavior
**Key outcomes**: FORM-BUG-7 confirmed (+1 day off for strings, +2 for Date objects); IST drift +5:30h/trip crosses day after ~4.4 trips
...
Record saved by BRT user: `Field5="2026-03-15T00:00:00"`, `Field6="2026-03-15T00:00:00"`, `Field7="2026-03-15"`.

| Field  | Config        | Raw on Load             | GFV (IST)                                        | GFV (BRT, from session 1)                        | DOM Display           |
| ------ | ------------- | ----------------------- | ------------------------------------------------ | ------------------------------------------------ | --------------------- |
| Field5 | D (ignoreTZ)  | `"2026-03-15T00:00:00"` | `"2026-03-15T00:00:00.000Z"` (fake Z)            | `"2026-03-15T00:00:00.000Z"` (same)              | `03/15/2026 12:00 AM` |
| Field6 | C (UTC)       | `"2026-03-15T00:00:00"` | `"2026-03-14T18:30:00.000Z"` (real UTC from IST) | `"2026-03-15T03:00:00.000Z"` (real UTC from BRT) | `03/15/2026 12:00 AM` |
| Field7 | A (date-only) | `"2026-03-15"`          | `"2026-03-15"`                                   | `"2026-03-15"`                                   | `03/15/2026`          |
...
### Test 4.4: SetFieldValue on Config A in IST — FORM-BUG-7 Confirmation
...
**Key outcomes**: FORM-BUG-7 confirmed for Configs A/B (-1 day stored); double-shift prediction wrong; getSaveValue() stores local midnight (NOT UTC offset) — matrix prediction for C/D corrected; Config C GetFieldValue correctly returns UTC (no bugs); Config D GetFieldValue appends fake Z FORM-BUG-5 confirmed (+5:30h drift per round-trip in IST)
...
| Field lookup (Config A) | filter `fieldType=13, enableTime=false, ignoreTimezone=false, useLegacy=false, enableInitialValue=false` | `["Field7"]` ✓                                                |

### Test 5.1: Calendar Popup — Config A, IST (Category 1-A-IST)

**Action**: Clicked calendar icon on Field7 (Config A: `enableTime=false`, `ignoreTimezone=false`, `useLegacy=false`) → March 2026 popup opened → clicked day 15 → popup closed immediately (no time tab — date-only field).
...
```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field7'),
    api: VV.Form.GetFieldValue('Field7'),
...
| GetFieldValue()                              | `"2026-03-14"`               | Same as raw — no fake Z (Config A unaffected by FORM-BUG-5) |
...
**FORM-BUG-7 confirmed**: Popup on date-only Config A in IST stores one day early. Display is correct (`03/15/2026`); stored value is `"2026-03-14"`.
...
**FORM-BUG-2 asymmetry NOT confirmed for Config A IST**: Both popup and typed input produce the same shift (-1 day = `"2026-03-14"`). The predicted divergence (popup → -2 days, typed → -1 day) does not materialize. Run test 2-A-IST to confirm typed input independently.
...
| V1/V2                   | `VV.Form.calendarValueService.useUpdatedCalendarValueLogic`                                             | `false` → V1 active ✓                                         |
| Field lookup (Config B) | filter `fieldType=13, enableTime=false, ignoreTimezone=true, useLegacy=false, enableInitialValue=false` | `["Field10"]` ✓                                               |

**Action**: Clicked calendar icon on Field10 (Config B: `enableTime=false`, `ignoreTimezone=true`, `useLegacy=false`) on form DateTest-000026 (fresh template instance) → March 2026 popup opened → clicked day 15 → popup closed immediately (no time tab — date-only field). Display showed `03/15/2026`.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field10'),
    api: VV.Form.GetFieldValue('Field10'),
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
});
// → { raw: "2026-03-14", api: "2026-03-14", isoRef: "2026-03-14T18:30:00.000Z" }
```
...
| Display in input                             | `03/15/2026`                 | Shows user-intended date ✓                                              |
| Raw stored value                             | `"2026-03-14"`               | **-1 day from intended March 15**                                       |
| GetFieldValue()                              | `"2026-03-14"`               | Same as raw — no fake Z (`enableTime=false`, FORM-BUG-5 not applicable) |
| isoRef (`new Date(2026,2,15).toISOString()`) | `"2026-03-14T18:30:00.000Z"` | Confirms IST active: local midnight = UTC 18:30 prev day                |
| Matrix Expected                              | `"2026-03-14"`               | **MATCH**                                                               |

**Findings:**

- **FORM-BUG-7 confirmed for Config B in IST**: Popup on date-only Config B stores one day early (`"2026-03-14"` for intended March 15). Identical result to Config A (Test 5.1).
- **`ignoreTimezone` has no effect on date-only storage**: Config B (`ignoreTimezone=true`) and Config A (`ignoreTimezone=false`) produce identical raw stored values and GetFieldValue returns. The `ignoreTimezone` flag only affects the `getCalendarFieldValue()` output path when `enableTime=true` (FORM-BUG-5 surface, Config D). For date-only fields, both configs go through the same `normalizeCalValue()` → `moment(input).toDate()` path.
- **FORM-BUG-5 confirmed absent**: GetFieldValue returns `"2026-03-14"` with no fake Z suffix. As expected — FORM-BUG-5 requires `enableTime=true`, which Config B does not have.
...
- **1-D-IST prediction corrected**: The matrix also predicted `"2026-03-14T18:30:00"` for Config D raw storage. Based on this result, Config D will store `"2026-03-15T00:00:00"` (same local midnight storage). The difference between C and D is only in GetFieldValue: Config D adds fake Z → `"2026-03-15T00:00:00.000Z"` (FORM-BUG-5), which causes +5:30h drift per round-trip.
- **Next test**: Run 1-D-IST (Config D, IST popup) to confirm `"2026-03-15T00:00:00"` storage and observe FORM-BUG-5 fake Z in GetFieldValue.
...
| V1/V2                   | `VV.Form.calendarValueService.useUpdatedCalendarValueLogic`                                            | `false` → V1 active ✓                                         |
| Field lookup (Config D) | filter `fieldType=13, enableTime=true, ignoreTimezone=true, useLegacy=false, enableInitialValue=false` | `["Field5"]` ✓                                                |

**Action**: Clicked calendar icon on Field5 (Config D: `enableTime=true`, `ignoreTimezone=true`, `useLegacy=false`) on form DateTest-000030 (fresh template instance) → March 2026 Date tab opened → scrolled page to reveal Set button → clicked day 15 → popup advanced to Time tab showing `12:00 AM` → clicked Set → popup closed. Display showed `03/15/2026 12:00 AM`.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field5'),
...
| GetFieldValue()                              | `"2026-03-15T00:00:00.000Z"` | **FORM-BUG-5 confirmed**: fake Z appended; real UTC should be `"2026-03-14T18:30:00.000Z"` |
| isoRef (`new Date(2026,2,15).toISOString()`) | `"2026-03-14T18:30:00.000Z"` | Confirms IST active ✓                                                                      |
...
- **FORM-BUG-5 confirmed in IST**: `getCalendarFieldValue()` appends fake Z to the local time string. The real UTC equivalent of IST midnight March 15 is `"2026-03-14T18:30:00.000Z"` (confirmed by isoRef), but GetFieldValue returns `"2026-03-15T00:00:00.000Z"` — UTC midnight March 15, which is 5:30h ahead of the correct value.
- **Round-trip drift in IST**: Each `SetFieldValue(GetFieldValue())` cycle advances the stored time by +5:30h (FORM-BUG-5). After ~4.4 trips, the date rolls over to the next day. This is the IST-specific variant of the +5:30h drift documented in FORM-BUG-5.
- **Config C vs Config D comparison complete**: Only difference is in GetFieldValue — Config C returns real UTC (`"2026-03-14T18:30:00.000Z"`), Config D returns fake Z (`"2026-03-15T00:00:00.000Z"`). Storage is identical. Result: FAIL (FORM-BUG-5).
...
**Purpose**: UTC+0 control test — confirm zero-drift behavior for Config A calendar popup; validates that FORM-BUG-7 shift is zero at UTC+0 (local midnight = UTC midnight)
**Key outcomes**: `"2026-03-15"` stored and returned — correct date, no shift; isoRef `"2026-03-15T00:00:00.000Z"` confirms UTC+0 active; FORM-BUG-7 zero-drift control confirmed
...
- **FORM-BUG-7 zero-drift control confirmed**: At UTC+0, `normalizeCalValue()` converts local midnight (2026-03-15 00:00 GMT) to a Date object whose `toISOString()` produces `"2026-03-15T00:00:00.000Z"`. Stripping the Z and extracting the date yields `"2026-03-15"` — the correct value. No day shift.
...
- **GetFieldValue Config A**: Returns the raw stored value unchanged. No `getCalendarFieldValue()` transformation applies to `enableTime=false` fields. Config A is not in the FORM-BUG-5 surface.
...
**Purpose**: UTC+0 control test for Config D (enableTime=true, ignoreTimezone=true) — confirm fake Z (FORM-BUG-5) is coincidentally correct at UTC+0 and raw storage is correct; validate round-trip stability claim
**Key outcomes**: `"2026-03-15T00:00:00"` stored (local midnight = UTC midnight at UTC+0); GetFieldValue returns `"2026-03-15T00:00:00.000Z"` (FORM-BUG-5 present but coincidentally correct); isoRef confirms UTC+0 active; PASS
...
| GetFieldValue()                              | `"2026-03-15T00:00:00.000Z"` | **FORM-BUG-5 present**: fake Z appended; at UTC+0 this is coincidentally the real UTC midnight |
| isoRef (`new Date(2026,2,15).toISOString()`) | `"2026-03-15T00:00:00.000Z"` | Confirms UTC+0 (GMT) active — local midnight = UTC midnight ✓                                  |
...
**Purpose**: Confirm typed input for date-only Configs A and B in IST stores -1 day (FORM-BUG-7); verify FORM-BUG-2 asymmetry (popup vs typed) is absent; confirm ignoreTZ no-op for date-only storage
**Key outcomes**: Both configs store `"2026-03-14"` (-1 day, FORM-BUG-7); GetFieldValue returns raw unchanged; FORM-BUG-2 absent; ignoreTZ no effect on date-only; isoRef confirms IST active
...
| -------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------- |
| Display in input                             | `03/15/2026`                 | Shows user-intended date ✓                                                      |
| Raw stored value                             | `"2026-03-14"`               | **FORM-BUG-7**: -1 day shift                                                    |
...
**Action**: Opened DateTest-000039 (fresh template instance). Clicked the input area of Field10 (row 8, Config B — `enableTime=false, ignoreTimezone=true`). Field entered segment-edit mode with "month" highlighted. Typed `03` → day segment; typed `15` → year segment; typed `2026` → pressed Tab. Field display after Tab: `03/15/2026`.
...
- **FORM-BUG-7 confirmed for Config B**: Typed `03/15/2026` → stored `"2026-03-14"` (-1 day). Same result as Config A (Test 8.1). `ignoreTimezone=true` has no effect on the date-only storage path — `normalizeCalValue()` and `getSaveValue()` behave identically for both configs when `enableTime=false`.
...
- **GetFieldValue unchanged**: Config B (`enableTime=false`) is outside the FORM-BUG-5 surface. No fake Z, no transformation — GFV returns raw value directly.
...
- **Knock-on note for Category 1/2 IST matrix**: Both popup (1-B-IST) and typed (2-B-IST) for Config B produce `"2026-03-14"`. FORM-BUG-2 asymmetry absent for Config B as well — consistent with Config A findings.
...
| :-------: | :--------: | :------: | :-------: | ---------------------------------------------------------------------------------------- |
|     A     |   false    |  false   |   false   | **TESTED** (Field7)                                                                      |
|     B     |   false    |   true   |   false   | **TESTED** (Field10)                                                                     |
|     C     |    true    |  false   |   false   | **TESTED** (Field6)                                                                      |
|     D     |    true    |   true   |   false   | **TESTED** (Field5)                                                                      |
|     E     |   false    |  false   |   true    | **TESTED** (Field12) — popup BRT (Test 9.1), typed BRT (Test 9.2), popup IST (Test 10.1) |
|     F     |   false    |   true   |   true    | **TESTED** (Field11) — popup tested (Test 9.3)                                           |
|     G     |    true    |  false   |   true    | **TESTED** (Field14) — popup BRT (Test 9.4)                                              |
|     H     |    true    |   true   |   true    | **TESTED** (Field13) — popup BRT (Test 9.5)                                              |
...
| 1-D-UTC0 |   D    | UTC+0 | Mar 15 12:00 AM | `"2026-03-15T00:00:00"` (fake Z coincidentally correct at UTC+0; FORM-BUG-5 present, no drift)                   | PASS ✓ (Test 7.1)                                |
| 1-A-IST  |   A    |  IST  | Mar 15          | `"2026-03-14"` (-1 day — single shift, popup = typed path)                                                       | FAIL — FORM-BUG-7 (Test 5.1)                     |
| 1-B-IST  |   B    |  IST  | Mar 15          | `"2026-03-14"` (-1 day — ignoreTZ no effect on date-only)                                                        | FAIL — FORM-BUG-7 (Test 5.2)                     |
...
| 1-D-IST  |   D    |  IST  | Mar 15 12:00 AM | `"2026-03-15T00:00:00"` (local midnight stored — GFV adds fake Z FORM-BUG-5; prediction corrected 2026-03-30)    | FAIL — FORM-BUG-5 (Test 5.4)                     |
| 1-E-BRT  |   E    |  BRT  | Mar 15          | `"2026-03-15T03:00:00.000Z"` (legacy UTC datetime; corrected 2026-03-31)                                         | FAIL — prediction wrong (Test 9.1)               |
| 1-F-BRT  |   F    |  BRT  | Mar 15          | `"2026-03-15T03:00:00.000Z"` (same as E-BRT — ignoreTZ no effect on legacy popup)                                | FAIL — FORM-BUG-2 (Test 9.3)                     |
| 1-G-BRT  |   G    |  BRT  | Mar 15 12:00 AM | `"2026-03-15T03:00:00.000Z"` (legacy DateTime popup closes without Time tab; raw UTC BRT midnight)               | FAIL — FORM-BUG-4 inert / no Time tab (Test 9.4) |
...
| 2-A-IST |   A    | IST | 03/15/2026          | `"2026-03-14"` (-1 day — string path, same as popup; FORM-BUG-2 absent)            | FAIL — FORM-BUG-7 (Test 8.1) |
| 2-B-IST |   B    | IST | 03/15/2026          | `"2026-03-14"` (same as 2-A-IST — ignoreTZ no effect on date-only)                 | FAIL — FORM-BUG-7 (Test 8.2) |
| 2-E-BRT |   E    | BRT | 03/15/2026          | `"2026-03-15"` (typed path — date-only; differs from popup — FORM-BUG-2 confirmed) | PASS ✓ (Test 9.2)            |
...
| ------------ | :------------: | -------- | :-: | -------------------------- | ---------------------------------------- |
| 5-A-BRT      | A (eT=f,iTz=f) | 3/1/2026 | BRT | `"2026-03-01"` or Date obj | DONE ✓ (Field2)                          |
...
| ------------ | :------------: | :-: | ------------------------------ | --------------------------------------------- |
| 6-A-BRT      | A (eT=f,iTz=f) | BRT | Date obj with current UTC time | DONE ✓ (Field1)                               |
...
| 13-after-roundtrip   | DB values after FORM-BUG-5 drift                                                  | NOT TESTED (save after round-trip, check DB) |
...
2. **HIGH — Category 1/2 IST (popup vs typed for Config A)**: Open form in IST, pick March 15 from popup on Field7, then type 03/15/2026 — compare stored values. Expected: popup → `"2026-03-13"`, typed → `"2026-03-14"` (different due to FORM-BUG-7 Date object vs string asymmetry).
...
Sessions 1–5 (2026-03-27 and 2026-03-30). Session 4 (IST) ran Tests 5.1–5.4: 1-A-IST and 1-B-IST confirmed FORM-BUG-7 (-1 day) for Configs A and B; 1-C-IST confirmed Config C stores local midnight (getSaveValue() formats as local time, not UTC offset); 1-D-IST confirmed FORM-BUG-5 fake Z in IST (+5:30h drift per round-trip). System TZ returned to BRT after Session 4. Session 5 (UTC+0/GMT) ran Test 6.1: 1-A-UTC0 confirmed zero-drift control — Config A popup stores correct date at UTC+0. Note: `Europe/London` is NOT equivalent to UTC+0 in March (BST = UTC+1 since March 29, 2026); use `GMT` timezone for UTC+0 testing on macOS.
...
- **`useLegacy=true`** — Emanuel does not currently have access to enable this setting. This is the main gap — the analysis document's FORM-BUG-2 (inconsistent popup vs typed handlers) may only manifest with legacy mode.
...
- **Preset/Current Date with DateTime fields** — Field1-4 are all date-only. Need DateTime fields with initial value configs to test Scenarios 5/6 with `enableTime=true`.

### Key findings to carry forward

1. **FORM-BUG-5 is the highest-severity confirmed bug**: The `getCalendarFieldValue()` function adds a fake literal `[Z]` when `enableTime=true && ignoreTimezone=true && !useLegacy`. This causes progressive date drift on every `SetFieldValue(GetFieldValue())` round-trip. Drift = timezone offset per trip.
2. **FORM-BUG-2 was NOT reproduced** with `useLegacy=false` — popup and typed input produce identical values.
...
VV.Form.GetFieldValue('Field5'); // processed return value
VV.Form.SetFieldValue('Field5', value); // set value
```

### Note on test numbering

Tests were written in execution order, not numerical order. The sequence in the document is:
2.0 (baseline) → 2.1 (popup) → 2.2 (round-trip) → 2.8 (cross-TZ analysis) → 2.9 (live Mumbai) → 2.7 (typed input) → 2.5 (Field10) → 2.6 (multiple trips) → 2.3 (reload) → 2.4 (DB evidence). This reflects the actual testing progression.
...
**Key outcomes**: All four legacy popup configs (E/F/G/H) confirmed. Legacy popup path stores full UTC datetime ISO string `"2026-03-15T03:00:00.000Z"` regardless of `enableTime` or `ignoreTimezone` flags — all four store identical format. Popup closes immediately after day selection for DateTime configs (G/H) — no Time tab despite `enableTime=true`. `useLegacy=true` correctly skips fake-Z branch in GetFieldValue (H confirmed). Typed input (2-E-BRT) stores `"2026-03-15"` (date-only) — **FORM-BUG-2 confirmed** for useLegacy=true. Legacy fields use plain text input widget (not Kendo masked DatePicker). 1-G-BRT (Config G, enableTime=true, ignoreTimezone=false, useLegacy=true) — legacy DateTime popup closes without Time tab on day click; stores raw UTC BRT midnight `"2026-03-15T03:00:00.000Z"`; same raw storage pattern as E/F despite enableTime=true. FORM-BUG-4 (Z-stripping) inert on popup path.
...
**Action:** Clicked calendar icon (`.k-icon.k-i-calendar` inside `.fd-cal-container`) adjacent to Field12. Popup opened showing March 2026. Clicked the cell with title "Sunday, March 15, 2026". Popup closed immediately (no time tab — `enableTime=false`). Field input displayed `03/15/2026`.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field12'),
    api: VV.Form.GetFieldValue('Field12'),
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
});
...
- **Date is correct**: `"2026-03-15T03:00:00.000Z"` = March 15, 2026 00:00:00 BRT — the selected date is preserved correctly. No date drift. FORM-BUG-7 is absent in BRT as expected.
- **Z suffix present**: FORM-BUG-4 (Legacy Save Format — removes Z) appears inert on the calendar popup path for useLegacy=true fields. The stored value retains the Z. Whether getSaveValue() is bypassed or whether it handles the value differently needs further code analysis.
...
- **CLAUDE.md outdated note**: The "What Has NOT Been Tested" section listed `useLegacy=true` as inaccessible. Field12/11/14/13 ARE accessible on the test form. That note should be removed in a future CLAUDE.md update.
- **Next test**: Run 2-E-BRT (typed input, Config E, BRT) — the paired test for FORM-BUG-2. If typed input stores `"2026-03-15"` (date-only string path) while popup stores `"2026-03-15T03:00:00.000Z"`, FORM-BUG-2 is confirmed for useLegacy=true.
...
**Action:** Discovered that Config E fields (`useLegacy=true`) render as a **plain HTML text input**, not the Kendo masked DatePicker used by modern configs (A/B/C/D). There are no separate month/day/year segments. Clicked Field12 at its center coordinate (scrolled into view via JS to resolve click-landing issues at the bottom of the form). Typed `03/15/2026` as a continuous string. Pressed Tab. Field retained display value `03/15/2026`.

**Captured values:**

```javascript
({
    displayValue: document.getElementById('Field12').value,
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field12'),
    api: VV.Form.GetFieldValue('Field12'),
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
});
...
- **No FORM-BUG-7**: BRT (UTC-3) is not affected by FORM-BUG-7 for date-only fields. The stored `"2026-03-15"` is correct.
- **Knock-on**: 2-F-BRT prediction `"2026-03-15"` remains valid — ignoreTZ has no effect on date-only typed input. 2-F-BRT will also confirm FORM-BUG-2 when paired with 1-F-BRT popup result.
- **Next test**: 1-F-BRT (popup, Config F) and 2-F-BRT (typed, Config F) to confirm FORM-BUG-2 holds for ignoreTZ=true legacy fields as well.
...
**Action:** Triggered click on calendar icon (`.k-icon.k-i-calendar` inside `.fd-cal-container`) adjacent to Field11 via JS `.click()` (required due to scroll position). Popup opened showing March 2026. Clicked the cell with title "Sunday, March 15, 2026". Popup closed immediately (no time tab — `enableTime=false`). Field input displayed `03/15/2026`. "Unsaved Changes" counter incremented from 18 to 19, confirming the field value was registered.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field11'),
    api: VV.Form.GetFieldValue('Field11'),
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
    displayValue: document.getElementById('Field11').value,
});
...
- **FORM-BUG-2 continues to apply**: Config F popup stores `"2026-03-15T03:00:00.000Z"` (UTC datetime). When 2-F-BRT is run (typed input), it is expected to store `"2026-03-15"` (date-only string) — same asymmetry as confirmed for Config E in Tests 9.1/9.2. This test alone does not confirm FORM-BUG-2 for Config F; the paired typed test (2-F-BRT) is required.
- **Z suffix present**: FORM-BUG-4 (Legacy Save Format — removes Z) is inert on the legacy popup path for date-only fields, same as observed for Config E. The stored value retains the Z.
- **Date is correct**: `"2026-03-15T03:00:00.000Z"` = March 15, 2026 00:00:00 BRT — no date drift. FORM-BUG-7 is absent in BRT as expected (UTC-3 midnight remains same UTC day).
- **Next test**: Run 2-F-BRT (typed input, Config F, BRT) to directly confirm FORM-BUG-2 for Config F — if typed stores `"2026-03-15"` while popup stores `"2026-03-15T03:00:00.000Z"`, FORM-BUG-2 is confirmed on the second legacy config.
...
**Action:** Triggered click on calendar icon (`.k-icon.k-i-calendar` inside `.fd-cal-container`) adjacent to Field14 via JS `.click()`. Popup opened showing March 2026. Clicked the cell with title "Sunday, March 15, 2026" via `td.click()`. Popup closed immediately — **no Time tab appeared** despite `enableTime=true`. Field input displayed `03/15/2026 12:00 AM`. The time defaulted to midnight local time (BRT 00:00 = 03:00 UTC) without any time selection step.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field14'),
    api: VV.Form.GetFieldValue('Field14'),
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
});
// → { raw: "2026-03-15T03:00:00.000Z", api: "2026-03-15T03:00:00.000Z", isoRef: "2026-03-15T03:00:00.000Z" }
```
...
| GetFieldValue()  | `"2026-03-15T03:00:00.000Z"` | Same as raw; no fake Z (ignoreTimezone=false, FORM-BUG-5 branch inactive) |
| isoRef           | `"2026-03-15T03:00:00.000Z"` | Confirms BRT (UTC-3) active: local midnight = 03:00:00 UTC                |
...
- **FORM-BUG-4 (Z-stripping) inert on popup path**: The raw value retains the Z suffix. FORM-BUG-4 (`getSaveValue()` strips Z for legacy DateTime) would only apply at form submission to the server, not to the in-memory `getValueObjectValue()` read.
- **No date drift**: `"2026-03-15T03:00:00.000Z"` = March 15, 2026 00:00:00 BRT — correct date. FORM-BUG-7 is absent in BRT (UTC-3 midnight remains same UTC day).
- **FORM-BUG-5 absent**: `ignoreTimezone=false` for Config G — fake-Z branch in `getCalendarFieldValue()` is not active. GetFieldValue returns the raw value unchanged.
...
**Action:** Triggered click on calendar icon (`.k-icon.k-i-calendar` inside `.fd-cal-container`) adjacent to Field13 via JS `.click()` (field scrolled into view first). Popup opened showing March 2026. Clicked the cell with title "Sunday, March 15, 2026" via `td.click()`. Popup closed immediately — **no Time tab appeared** despite `enableTime=true`, consistent with Config G (Test 9.4). Field input displayed `03/15/2026 12:00 AM`. Time defaulted to midnight without any time selection step.

**Captured values:**

```javascript
({
    raw: VV.Form.VV.FormPartition.getValueObjectValue('Field13'),
    api: VV.Form.GetFieldValue('Field13'),
    displayValue: document.getElementById('Field13').value,
    isoRef: new Date(2026, 2, 15, 0, 0, 0).toISOString(),
});
...
- **No date drift**: `"2026-03-15T03:00:00.000Z"` = March 15, 2026 00:00:00 BRT — correct date. FORM-BUG-7 absent in BRT as expected.
...
**Action:** Triggered click on calendar icon (`.k-icon.k-i-calendar` inside `.fd-cal-container`) adjacent to Field12 via JS `.click()`. Popup opened showing March 2026. Clicked the cell with title "Sunday, March 15, 2026" via `td.click()`. Popup closed immediately — **no Time tab** (legacy date-only, `enableTime=false`). Field input displayed `03/15/2026`.
...
- **No fake Z**: GetFieldValue returns the raw value unchanged. `enableTime=false` means the fake-Z branch in `getCalendarFieldValue()` is not active. FORM-BUG-5 is absent here (as expected for this config).
- **FORM-BUG-2 applies when paired with 2-E-IST**: The popup stores `"2026-03-14T18:30:00.000Z"` (UTC datetime). Typed input (Test 2-E-IST, not yet run) is expected to store `"2026-03-14"` (date-only, same as 2-E-BRT). This asymmetry — same field, same date, different storage format — is FORM-BUG-2 applied to IST.
...
- **Next test**: 2-E-IST (typed input, Config E, IST) to confirm FORM-BUG-2 manifests — typed input should store `"2026-03-14"` (date-only) vs popup's `"2026-03-14T18:30:00.000Z"`. Then 1-F-IST to verify ignoreTZ is a no-op on the legacy IST popup.
...
- 2026-03-31 [TC-2-E-IST Run 1](runs/tc-2-E-IST-run-1.md) — IST — FAIL-1 — FORM-BUG-7 in legacy typed input; stores "2026-03-14" (-1 day); FORM-BUG-2 format diff confirmed
...
- 2026-03-31 [TC-2-D-IST Run 1](runs/tc-2-D-IST-run-1.md) — IST — FAIL-1 — Config D typed input: same storage as C-IST; FORM-BUG-5 fake Z in GFV confirmed
- 2026-03-31 [TC-2-F-IST Run 1](runs/tc-2-F-IST-run-1.md) — IST — FAIL-1 — legacy typed Config F stores date-only `"2026-03-14"` (FORM-BUG-7); differs from popup format (FORM-BUG-2)
...
**Key outcomes**: PASS — typed input stores local midnight correctly; FORM-BUG-2 confirmed (popup vs typed inconsistency).

- 2026-03-31 [TC-2-G-IST Run 1](runs/tc-2-G-IST-run-1.md) — IST — PASS — legacy DateTime typed stores local midnight; FORM-BUG-2 confirmed vs popup
...
**Key outcomes**: PASS — both configs store correctly via getSaveValue; FORM-BUG-2 confirmed (popup vs typed format inconsistency).

- 2026-03-31 [TC-2-F-BRT Run 1](runs/tc-2-F-BRT-run-1.md) — BRT — PASS — legacy typed Config F stores date-only; ignoreTZ no-op; FORM-BUG-2 confirmed vs popup
- 2026-03-31 [TC-2-G-BRT Run 1](runs/tc-2-G-BRT-run-1.md) — BRT — PASS — legacy DateTime typed stores local midnight "2026-03-15T00:00:00"; FORM-BUG-2 confirmed vs popup
...
- 2026-03-31 [TC-3-D-BRT-BRT Run 2](runs/tc-3-D-BRT-BRT-run-2.md) — BRT — PASS (FAIL-3) — Config D server reload: raw value unchanged; FORM-BUG-5 fake Z confirmed in GFV on reload
...
- 2026-03-31 [TC-3-D-BRT-BRT Run 3](runs/tc-3-D-BRT-BRT-run-3.md) — BRT — PASS (FAIL-3) — full save-then-reload on fresh form; FORM-BUG-5 active pre-save and post-reload

## Session 2026-04-01 (IST)

**Purpose**: Cross-TZ reload verification — BRT-saved records opened in IST.
**Key outcomes**: Config D raw TZ-invariant (FORM-BUG-5 on GFV); Config A date-only survives cross-TZ reload (FORM-BUG-7 prediction disproved).

- 2026-04-01 [TC-3-D-BRT-IST Run 2](runs/tc-3-D-BRT-IST-run-2.md) — IST — FAIL-3 — Config D cross-TZ reload: raw value stable, display correct; FORM-BUG-5 fake Z on GFV (corrects Run 1 PASS)
- 2026-04-01 [TC-3-A-BRT-IST Run 1](runs/tc-3-A-BRT-IST-run-1.md) — IST — PASS — Config A cross-TZ reload: date-only "2026-03-15" survives IST reload; FORM-BUG-7 prediction disproved
...
- 2026-04-01 [TC-3-D-BRT-IST Run 3](runs/tc-3-D-BRT-IST-run-3.md) — IST — FAIL-3 — Config D on fresh record (DateTest-000080); FORM-BUG-5 confirmed; raw format changed to US on reload
- 2026-04-01 [TC-3-C-BRT-IST Run 1](runs/tc-3-C-BRT-IST-run-1.md) — IST — FAIL-3,FAIL-4 — Config C cross-TZ: DateTime reinterpreted as IST local; GFV 8.5h shift (FORM-BUG-1+#4)

## Session 2026-04-01 (BRT — IST→BRT reload)

**Purpose**: Reverse cross-TZ reload — IST-saved record (DateTest-000084) opened in BRT.
**Key outcomes**: Config D raw TZ-invariant (FORM-BUG-5 on GFV); bidirectional cross-TZ verification complete.

- 2026-04-01 [TC-3-D-IST-BRT Run 1](runs/tc-3-D-IST-BRT-run-1.md) — BRT — PASS (FAIL-3) — Config D IST→BRT reload: raw value unchanged, display correct; FORM-BUG-5 fake Z on GFV; confirms bidirectional TZ-invariance
...
- 2026-04-01 [TC-2-A-BRT Run 2](runs/tc-2-A-BRT-run-2.md) — BRT — PASS — Config A typed input via Playwright CLI; matches popup, FORM-BUG-2 absent
...
**Key outcomes**: FORM-BUG-7 confirmed permanent — wrong day baked in during IST save persists on BRT reload.

- 2026-04-01 [TC-3-A-IST-BRT Run 1](runs/tc-3-A-IST-BRT-run-1.md) — BRT — FAIL-3 — Config A IST→BRT reload; FORM-BUG-7 wrong day from IST save persists (`"2026-03-14"` instead of `"2026-03-15"`)

## Session 2026-04-01 (IST — Playwright CLI, Preset Date)

**Purpose**: Verify preset date initial values in IST — does FORM-BUG-7 fire on form init path?
**Key outcomes**: FORM-BUG-7 confirmed on preset path. Major scope expansion: every UTC+ form instance with date-only presets carries wrong internal UTC date.

- 2026-04-01 [TC-5-A-IST Run 1](runs/tc-5-A-IST-run-1.md) — IST — FAIL-3 — Config A preset in IST; FORM-BUG-7 on init path: Date obj UTC = `"2026-02-28"`, display correct `03/01/2026`, save would corrupt

## Session 2026-04-01 (BRT — Playwright CLI, GetFieldValue Empty)

**Purpose**: Test GetFieldValue on empty fields — scope FORM-BUG-6 beyond Config D.
**Key outcomes**: FORM-BUG-6 confirmed D-only; Config A empty field returns `""` correctly.

- 2026-04-01 [TC-8-A-empty Run 1](runs/tc-8-A-empty-run-1.md) — BRT — PASS — Config A empty field returns `""` (not "Invalid Date"); FORM-BUG-6 absent for enableTime=false
...
**Key outcomes**: GDOC correct (real UTC in toISOString); FORM-BUG-5 re-confirmed via GFV comparison (fake Z vs real Z, 3h gap).

- 2026-04-01 [TC-8B-D-BRT Run 1](runs/tc-8B-D-BRT-run-1.md) — BRT — PASS + FAIL-1 — Config D GDOC correct; GFV comparison re-confirms FORM-BUG-5 fake Z (3h discrepancy)
- 2026-04-01 [TC-8-H-BRT Run 1](runs/tc-8-H-BRT-run-1.md) — BRT — PASS — Config H GFV returns raw unchanged; useLegacy=true bypasses FORM-BUG-5 fake-Z branch
...
- 2026-04-01 [TC-8-C-empty Run 1](runs/tc-8-C-empty-run-1.md) — BRT — FAIL-1 — Config C empty GFV THROWS RangeError; FORM-BUG-6 variant worse than Config D (throws vs returns string)
...
- 2026-04-01 [TC-9-H-BRT-1 Run 1](runs/tc-9-H-BRT-1-run-1.md) — BRT — PASS — Config H GFV round-trip stable (0 drift); useLegacy=true prevents FORM-BUG-5 fake-Z drift
...
- 2026-04-02 [TC-3-H-BRT-BRT Run 1](runs/tc-3-H-BRT-BRT-run-1.md) — BRT — PASS — Config H legacy DateTime + ignoreTZ same-TZ reload; useLegacy bypasses FORM-BUG-5
...
- 2026-04-02 [TC-3-H-BRT-IST Run 1](runs/tc-3-H-BRT-IST-run-1.md) — IST — PASS — Config H legacy DateTime + ignoreTZ cross-TZ reload; useLegacy bypasses FORM-BUG-5 in IST
- 2026-04-02 [TC-12-near-midnight-1-IST Run 1](runs/tc-12-near-midnight-1-IST-run-1.md) — IST — FAIL-1 — ISO+Z near midnight: no day cross in IST (contrast BRT); FORM-BUG-5 fake Z on GFV
...
- 2026-04-02 [TC-3-B-IST-BRT Run 1](runs/tc-3-B-IST-BRT-run-1.md) — BRT — FAIL-3 — Config B FORM-BUG-7 baked in during IST save; ignoreTZ inert. Cat 3 complete (14P/4F)
...
- 2026-04-03 [TC-1-C-BRT Run 2](runs/tc-1-C-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-4
- 2026-04-03 [TC-1-D-BRT Run 2](runs/tc-1-D-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-near-midnight-1 Run 1](runs/tc-12-near-midnight-1-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-near-midnight-2 Run 1](runs/tc-12-near-midnight-2-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-dst-transition Run 1](runs/tc-12-dst-transition-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-year-boundary Run 1](runs/tc-12-year-boundary-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-leap-day Run 1](runs/tc-12-leap-day-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-empty-value Run 1](runs/tc-12-empty-value-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-6
- 2026-04-03 [TC-12-invalid-string Run 1](runs/tc-12-invalid-string-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-6
- 2026-04-03 [TC-12-far-future Run 1](runs/tc-12-far-future-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-pre-epoch Run 1](runs/tc-12-pre-epoch-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-2-C-BRT Run 2](runs/tc-2-C-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-4
- 2026-04-03 [TC-2-D-BRT Run 2](runs/tc-2-D-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-3-A-IST-BRT Run 2](runs/tc-3-A-IST-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-3-B-IST-BRT Run 2](runs/tc-3-B-IST-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-3-C-BRT-BRT Run 4](runs/tc-3-C-BRT-BRT-run-4.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-4
- 2026-04-03 [TC-3-D-BRT-BRT Run 4](runs/tc-3-D-BRT-BRT-run-4.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-3-D-IST-BRT Run 2](runs/tc-3-D-IST-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-3-C-IST-BRT Run 2](runs/tc-3-C-IST-BRT-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-1, FORM-BUG-4
- 2026-04-03 [TC-7-D-dateObj Run 1](runs/tc-7-D-dateObj-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-isoZ Run 1](runs/tc-7-D-isoZ-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-isoNoZ Run 1](runs/tc-7-D-isoNoZ-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-dateOnly Run 1](runs/tc-7-D-dateOnly-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-usFormat Run 1](runs/tc-7-D-usFormat-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-usFormatTime Run 1](runs/tc-7-D-usFormatTime-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-epoch Run 1](runs/tc-7-D-epoch-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-8-C-empty Run 2](runs/tc-8-C-empty-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-6
...
- 2026-04-03 [TC-8-D-BRT Run 1](runs/tc-8-D-BRT-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-8-D-empty Run 1](runs/tc-8-D-empty-run-1.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-6
...
- 2026-04-03 [TC-9-D-BRT-1 Run 2](runs/tc-9-D-BRT-1-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-9-D-BRT-8 Run 2](runs/tc-9-D-BRT-8-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-9-D-BRT-10 Run 2](runs/tc-9-D-BRT-10-run-2.md) — BRT — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-1-C-IST Run 2](runs/tc-1-C-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-4
- 2026-04-03 [TC-1-A-IST Run 2](runs/tc-1-A-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-1-B-IST Run 2](runs/tc-1-B-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-1-D-IST Run 2](runs/tc-1-D-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-near-midnight-1-IST Run 2](runs/tc-12-near-midnight-1-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-near-midnight-2-IST Run 2](runs/tc-12-near-midnight-2-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-year-boundary-IST Run 2](runs/tc-12-year-boundary-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-12-leap-day-IST Run 2](runs/tc-12-leap-day-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-2-A-IST Run 2](runs/tc-2-A-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-2-B-IST Run 2](runs/tc-2-B-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-2-C-IST Run 2](runs/tc-2-C-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-4
- 2026-04-03 [TC-2-D-IST Run 2](runs/tc-2-D-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-2-E-IST Run 2](runs/tc-2-E-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-2-F-IST Run 2](runs/tc-2-F-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
...
- 2026-04-03 [TC-3-C-BRT-IST Run 2](runs/tc-3-C-BRT-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-1, FORM-BUG-4
- 2026-04-03 [TC-3-D-BRT-IST Run 4](runs/tc-3-D-BRT-IST-run-4.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-5-A-IST Run 2](runs/tc-5-A-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-5-D-IST Run 2](runs/tc-5-D-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-6-A-IST Run 2](runs/tc-6-A-IST-run-2.md) — IST — PASS — Firefox confirms Config A Current Date stores correctly
- 2026-04-03 [TC-6-D-IST Run 2](runs/tc-6-D-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-A-dateOnly-IST Run 1](runs/tc-7-A-dateOnly-IST-run-1.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-7-A-dateObj-IST Run 1](runs/tc-7-A-dateObj-IST-run-1.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-7
- 2026-04-03 [TC-7-D-dateObj-IST Run 2](runs/tc-7-D-dateObj-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-isoZ-IST Run 2](runs/tc-7-D-isoZ-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-7-D-isoNoZ-IST Run 2](runs/tc-7-D-isoNoZ-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-8-D-IST Run 1](runs/tc-8-D-IST-run-1.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-8-D-empty-IST Run 2](runs/tc-8-D-empty-IST-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-6
...
- 2026-04-03 [TC-9-D-IST-1 Run 2](runs/tc-9-D-IST-1-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
- 2026-04-03 [TC-9-D-IST-5 Run 2](runs/tc-9-D-IST-5-run-2.md) — IST — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-1-D-UTC0 Run 2](runs/tc-1-D-UTC0-run-2.md) — UTC0 — FAIL — Firefox confirms known bug — FORM-BUG-5
...
- 2026-04-03 [TC-1-D-UTC0 Run 3](runs/tc-1-D-UTC0-run-3.md) — UTC0 — FAIL — Firefox confirms known bug — FORM-BUG-5
...
**Key outcomes**: 3 PASS, 2 FAIL. FORM-BUG-7 confirmed for B-IST. FORM-BUG-5 confirmed for D-BRT. Config C DateTime presets are TZ-independent. DateTime presets bypass parseDateString truncation.
...
- 2026-04-03 [TC-5-D-BRT Run 1](runs/tc-5-D-BRT-run-1.md) — BRT — FAIL — FORM-BUG-5 fake Z on GFV (-3h shift in BRT)
- 2026-04-03 [TC-5-B-IST Run 1](runs/tc-5-B-IST-run-1.md) — IST — FAIL — FORM-BUG-7 stores Feb 28 instead of Mar 1
...
**Key outcomes**: 3 PASS, 1 FAIL. Config A FORM-BUG-7 boundary confirmed at UTC0. Config C TZ-independence confirmed across all 3 TZs. FORM-BUG-5 structurally present but invisible at UTC0. PST (UTC-8) confirms FORM-BUG-7 absent in deep UTC-.

- 2026-04-03 [TC-5-A-UTC0 Run 1](runs/tc-5-A-UTC0-run-1.md) — UTC0 — PASS — FORM-BUG-7 boundary control (local midnight = UTC midnight)
- 2026-04-03 [TC-5-C-UTC0 Run 1](runs/tc-5-C-UTC0-run-1.md) — UTC0 — PASS — Config C TZ-independent (identical to BRT/IST)
- 2026-04-03 [TC-5-D-UTC0 Run 1](runs/tc-5-D-UTC0-run-1.md) — UTC0 — FAIL — FORM-BUG-5 fake Z (0h shift, numerically invisible at UTC+0)
- 2026-04-03 [TC-5-A-PST Run 1](runs/tc-5-A-PST-run-1.md) — PST — PASS — UTC-8 confirms FORM-BUG-7 absent in negative-offset TZs
...
**Key outcomes**: 4 PASS, 2 FAIL. Legacy date-only (E/F) identical to non-legacy (A/B). Legacy DateTime (G/H) stores raw Date unchanged — no FORM-BUG-5 fake Z. FORM-BUG-7 confirmed on legacy preset path in IST. **Category 5 complete: 18/18 (11P, 7F).**
...
- 2026-04-03 [TC-5-H-BRT Run 1](runs/tc-5-H-BRT-run-1.md) — BRT — PASS — Legacy DateTime + ignoreTZ: no FORM-BUG-5 fake Z (useLegacy=true safe)
- 2026-04-03 [TC-5-E-IST Run 1](runs/tc-5-E-IST-run-1.md) — IST — FAIL — FORM-BUG-7 on legacy preset (useLegacy does not protect)
- 2026-04-03 [TC-5-F-IST Run 1](runs/tc-5-F-IST-run-1.md) — IST — FAIL — FORM-BUG-7 on legacy preset (neither ignoreTZ nor useLegacy protects)
...
**Key outcomes**: 3 PASS, 1 FAIL. Current Date init path (`new Date()`) correct for all configs. FORM-BUG-5 fires on Config D GFV (-3h in BRT). Config A UTC0 completes 3-TZ spectrum.
...
- 2026-04-03 [TC-6-D-BRT Run 1](runs/tc-6-D-BRT-run-1.md) — BRT — FAIL — FORM-BUG-5 fake Z on GFV (-3h shift)
...
**Key outcomes**: 8 PASS, 0 FAIL. **Category 6 complete: 15/15 (13P, 2F).** All current date fields correct — `new Date()` bypasses all parsing bugs. Legacy configs safe from FORM-BUG-5. Cross-midnight edge demonstrated in IST.
...
- 2026-04-03 [TC-6-H-BRT Run 1](runs/tc-6-H-BRT-run-1.md) — BRT — PASS — Legacy avoids FORM-BUG-5 (compare 6-D-BRT FAIL)
...
- 2026-04-03 [TC-6-E-IST Run 1](runs/tc-6-E-IST-run-1.md) — IST — PASS — No FORM-BUG-7 (compare preset 5-E-IST FAIL)
- 2026-04-03 [TC-6-F-IST Run 1](runs/tc-6-F-IST-run-1.md) — IST — PASS — No FORM-BUG-7 on current date path
...
**Purpose**: Config B SetFieldValue tests — 3 BRT formats + 2 IST formats. Verify ignoreTZ inert on date-only, confirm FORM-BUG-7 in IST.
**Key outcomes**: 3 PASS, 2 FAIL. Config B identical to Config A for date-only — `ignoreTimezone` inert. FORM-BUG-7 confirmed in IST (-1 day string, -2 days dateObj). Cat 7 PASS 15→18, FAIL 5→7, PENDING 19→14.
...
- 2026-04-03 [TC-7-B-dateOnly-IST Run 1](runs/tc-7-B-dateOnly-IST-run-1.md) — IST — FAIL-1 — FORM-BUG-7 stores -1 day
- 2026-04-03 [TC-7-B-dateObj-IST Run 1](runs/tc-7-B-dateObj-IST-run-1.md) — IST — FAIL-1 — FORM-BUG-7 double-shift stores -2 days
...
**Key outcomes**: 6 PASS, 2 FAIL. Legacy date-only (E/F) identical to non-legacy — useLegacy inert, FORM-BUG-7 fires in IST. Legacy DateTime (G/H) GFV returns raw value directly — no UTC conversion (unlike C), no fake Z (unlike D). useLegacy=true is the FORM-BUG-5 countermeasure. Cat 7 PASS 18→24, FAIL 7→9, PENDING 14→6.

- 2026-04-03 [TC-7-E-dateOnly-BRT Run 1](runs/tc-7-E-dateOnly-BRT-run-1.md) — BRT — PASS — Legacy date-only same as Config A
- 2026-04-03 [TC-7-E-dateOnly-IST Run 1](runs/tc-7-E-dateOnly-IST-run-1.md) — IST — FAIL-1 — FORM-BUG-7 -1 day, useLegacy no protection
- 2026-04-03 [TC-7-F-dateOnly-IST Run 1](runs/tc-7-F-dateOnly-IST-run-1.md) — IST — FAIL-1 — FORM-BUG-7 -1 day, neither flag protects
...
- 2026-04-03 [TC-7-H-isoNoZ-BRT Run 1](runs/tc-7-H-isoNoZ-BRT-run-1.md) — BRT — PASS — No fake Z (useLegacy protects from FORM-BUG-5)
- 2026-04-03 [TC-7-H-isoZ-BRT Run 1](runs/tc-7-H-isoZ-BRT-run-1.md) — BRT — PASS — Shifted, no fake Z (FORM-BUG-5 absent)
...
**Key outcomes**: 6 PASS, 1 FAIL. Legacy date-only (E/F) identical to A/B. Legacy DateTime (G/H) returns raw — useLegacy skips UTC conversion. 8-G-BRT prediction corrected (raw, not real UTC). FORM-BUG-5 invisible at UTC+0. V2 bypasses all GFV transformations. Cat 8: 12P/6F/1P remaining.

- 2026-04-03 [TC-8-C-UTC0 Run 1](runs/tc-8-C-UTC0-run-1.md) — UTC0 — PASS — Real UTC trivially correct at UTC+0
- 2026-04-03 [TC-8-D-UTC0 Run 1](runs/tc-8-D-UTC0-run-1.md) — UTC0 — FAIL — FORM-BUG-5 fake Z, coincidentally correct (0h drift)
...
- 2026-04-03 [TC-8-V2 Run 1](runs/tc-8-V2-run-1.md) — IST — PASS — V2 bypasses all GFV transformations, FORM-BUG-5 absent
...
**Key outcomes**: 8 PASS, 1 FAIL. GDOC returns correct Date objects for all configs. FORM-BUG-7 upstream corrupts stored value for 8B-A-IST. Legacy configs identical to non-legacy. Cat 8B complete: 11P/1F/0P.

- 2026-04-03 [TC-8B-A-BRT Run 1](runs/tc-8B-A-BRT-run-1.md) — BRT — PASS — Date-only GDOC correct at BRT midnight
- 2026-04-03 [TC-8B-A-IST Run 1](runs/tc-8B-A-IST-run-1.md) — IST — FAIL — FORM-BUG-7 upstream: stored Mar 14, GDOC reads correctly
...
- 2026-04-03 [TC-8B-C-IST Run 1](runs/tc-8B-C-IST-run-1.md) — IST — PASS — DateTime safe in IST, no FORM-BUG-7
...
**Key outcomes**: 3 PASS, 6 FAIL. FORM-BUG-5 drift confirmed proportional to TZ offset across 5 TZs. PST corrected to PDT (-7h, not -8h). 9-B-IST reveals FORM-BUG-7 drift on date-only round-trips in UTC+. Legacy configs (E, G) stable. **Cat 9 complete: 9P/11F/0P.**

- 2026-04-03 [TC-9-D-BRT-3 Run 1](runs/tc-9-D-BRT-3-run-1.md) — BRT — FAIL — FORM-BUG-5: 3×-3h = -9h drift
...
- 2026-04-03 [TC-9-D-IST-8 Run 1](runs/tc-9-D-IST-8-run-1.md) — IST — FAIL — FORM-BUG-5: 8×+5:30h = +44h drift
- 2026-04-03 [TC-9-D-IST-10 Run 1](runs/tc-9-D-IST-10-run-1.md) — IST — FAIL — FORM-BUG-5: 10×+5:30h = +55h drift
- 2026-04-03 [TC-9-B-IST Run 1](runs/tc-9-B-IST-run-1.md) — IST — FAIL — FORM-BUG-7: date-only -1 day per SFV in IST
