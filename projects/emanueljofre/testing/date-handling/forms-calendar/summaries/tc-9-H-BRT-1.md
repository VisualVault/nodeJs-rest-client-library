# TC-9-H-BRT-1 — Summary

**Spec**: [tc-9-H-BRT-1.md](tasks/date-handling/forms-calendar/test-cases/tc-9-H-BRT-1.md)
**Current status**: PASS — last run 2026-04-09 (BRT, Chromium)
**Bug surface**: none — useLegacy=true prevents Bug #5 round-trip drift

## Run History

| Run | Date       | TZ  | Outcome | File                                   |
| --- | ---------- | --- | ------- | -------------------------------------- |
| 1   | 2026-04-01 | BRT | PASS    | [run-1](../runs/tc-9-H-BRT-1-run-1.md) |
| 2   | 2026-04-03 | BRT | PASS    | [run-2](../runs/tc-9-H-BRT-1-run-2.md) |
| 3   | 2026-04-09 | BRT | PASS    | [run-3](../runs/tc-9-H-BRT-1-run-3.md) |

## Current Interpretation

Run 3 (2026-04-09, Chromium): PASS. Cross-browser verification in progress.

## Next Action

Run 9-H-IST-1 to confirm zero drift is TZ-independent.
