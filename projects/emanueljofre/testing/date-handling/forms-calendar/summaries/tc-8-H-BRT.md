# TC-8-H-BRT — Summary

**Spec**: [tc-8-H-BRT.md](tasks/date-handling/forms-calendar/test-cases/tc-8-H-BRT.md)
**Current status**: PASS — last run 2026-04-09 (BRT, Chromium)
**Bug surface**: none — control test confirming Bug #5 is absent for useLegacy=true

## Run History

| Run | Date       | TZ  | Outcome | File                                 |
| --- | ---------- | --- | ------- | ------------------------------------ |
| 1   | 2026-04-01 | BRT | PASS    | [run-1](../runs/tc-8-H-BRT-run-1.md) |
| 2   | 2026-04-03 | BRT | PASS    | [run-2](../runs/tc-8-H-BRT-run-2.md) |
| 3   | 2026-04-09 | BRT | PASS    | [run-3](../runs/tc-8-H-BRT-run-3.md) |

## Current Interpretation

Run 3 (2026-04-09, Chromium): PASS. Cross-browser verification in progress.

## Next Action

No further action for this TC — closed PASS. Run 8-H-empty next to test whether `useLegacy=true` also prevents Bug #6 (Invalid Date for empty fields).
