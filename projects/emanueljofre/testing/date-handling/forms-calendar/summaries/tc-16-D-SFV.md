...
|---|---|---|---|---|
| 1 | 2026-04-13 | BRT | PASS (FAIL-3 Bug #5, vvdemo) | [run-1](../runs/tc-16-D-SFV-run-1.md) |

## Current Interpretation

Cross-env comparison complete. On vv5dev (server PDT, UTC-7) and vvdemo (server BRT, UTC-3), Config D DateTime+iTZ via SetFieldValue produces identical API responses: field5=`"2026-03-15T14:30:00Z"`. The ignoreTimezone=true path preserves local time without conversion on both servers. Client FORM-BUG-5 (fake Z) is present on both environments — client-side bug independent of server timezone. Server TZ does not affect the form save pipeline for Config D.
