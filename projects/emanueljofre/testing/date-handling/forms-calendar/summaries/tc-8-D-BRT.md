# TC-8-D-BRT — Summary

**Spec**: [tc-8-D-BRT.md](tasks/date-handling/forms-calendar/test-cases/tc-8-D-BRT.md)
**Current status**: FAIL — last run 2026-04-09 (BRT, Chromium)
**Bug surface**: Bug #5 — GFV returns fake Z

## Run History

| Run | Date       | TZ  | Outcome | Notes                                      |
| --- | ---------- | --- | ------- | ------------------------------------------ |
| L2  | 2026-04-02 | —   | FAIL    | Playwright Layer 2 regression verification |
| 1   | 2026-04-03 | BRT | FAIL    | [run-1](../runs/tc-8-D-BRT-run-1.md)       |
| 2   | 2026-04-09 | BRT | FAIL    | [run-2](../runs/tc-8-D-BRT-run-2.md)       |

## Current Interpretation

Run 2 (2026-04-09, Chromium): FAIL. Cross-browser verification in progress.

## Next Action

No further action — behavior characterized.
