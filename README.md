...
(3) Install NGROK using the instructions for your OS
(4) From a terminal window in the NGROK directory, enter the command below which assumes your VisualVault Node.Js Client Library app is listening on port 3000 which is the default.  The port number can be changed within the app.js file.

```shell
    ngrok http 3000
```
(5) If using NGROK as a secure reverse proxy to your machine, copy the NGROK Public URL generated by the command.
(6) Past the public development machine URL into your VisualVault user preferences screen's Development Node Server field and save.
