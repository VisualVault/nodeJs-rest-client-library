    ...
    for (const dash of linksToTry) {
        ...
        const record = await page.evaluate(() => {
            const checkbox = document.querySelector(
                '.rgMasterTable tbody input[type="checkbox"][dhid]'
            );
...
async function discoverGuids(page) {
    ...
    const guids = await page.evaluate(() => {
        ...
        for (const script of scripts) {
            ...
            const xcidMatch = src.match(/xcid['":\s]*=?\s*['"]?([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/i);
            const xcdidMatch = src.match(/xcdid['":\s]*=?\s*['"]?([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/i);
            if (xcidMatch && xcdidMatch) {
                return { xcid: xcidMatch[1], xcdid: xcdidMatch[1] };
            }
        }
