# TC-DB-2-G — Config G, Date Accuracy: dashboard matches stored legacy DateTime value
...
| 2   | Read Field14 grid cell | DateTest-000890, column "Field14" | `3/15/2026 2:30 PM` — server renders stored UTC value `2026-03-15T14:30:00Z` directly | ☐   |

## Fail Conditions

1. **FAIL-1 (Wrong time):** Field14 shows a different time than `2:30 PM`.
    - Interpretation: Server applying TZ conversion or `useLegacy` flag affecting server-side rendering.

2. **FAIL-2 (Wrong date):** Field14 shows a date other than `3/15/2026`.
...
| Reference              | Location                                      |
| ---------------------- | --------------------------------------------- |
| Matrix row             | `matrix.md` — row `db-2-G`                    |
| Run history            | `summaries/tc-db-2-G.md`                      |
| Bug analysis           | `analysis.md` § 3 (Bug Surface in Dashboards) |
| WS stored value source | `../web-services/matrix.md` — WS-1 Config G   |
| Cross-reference (DB-1) | `test-cases/tc-db-1-G.md` — format verified   |
