...
Calendar fields may display or store incorrect dates. Five distinct bugs have been identified, affecting 7 of 8 common scenarios.
...
| Reports & Queries | Database queries filtering by date may return unexpected results |
...
| Bug # | Name | Root Cause |
|-------|------|------------|
| 1 | Timezone Marker Stripping | `parseDateString()` removes "Z" suffix from UTC dates |
| 2 | Inconsistent User Input Handlers | Calendar popup and typed input use different save logic |
| 3 | Hardcoded Parameters | `initCalendarValueV2()` ignores actual field settings |
| 4 | Legacy Save Format | `getSaveValue()` removes "Z" suffix when saving |
| 5 | Inconsistent Developer API | `SetFieldValue` and `GetFieldValue` have different transformation logic |

### Key Insight

Only one scenario works correctly: **New Form with "Current Date" Default**. All other date entry methods—including developer API calls—have at least one bug.
...
| Scenario | Bug #1 | Bug #2 | Bug #3 | Bug #4 | Bug #5 | Status |
|----------|:------:|:------:|:------:|:------:|:------:|--------|
| 1. Calendar Popup | - | ✗ | - | - | - | ⚠️ Partial Issue |
| 2. Typed Input | ✗ | ✗ | - | ✗ | - | ❌ Broken |
| 3. Saved Data | ✗ | - | ✗ | ✗ | - | ❌ Broken |
| 4. URL Parameter | ✗ | - | - | ✗ | - | ❌ Broken |
| 5. Preset Date | ✗ | - | ✗ | ✗ | - | ❌ Broken |
| 6. Current Date | - | - | - | - | - | ✅ Works |
| 7. SetFieldValue | - | ✗ | - | - | ✗ | ⚠️ Partial Issue |
| 8. GetFieldValue | - | - | - | - | ✗ | ⚠️ Inconsistent |
