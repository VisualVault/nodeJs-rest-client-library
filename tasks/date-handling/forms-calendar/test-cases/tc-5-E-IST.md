# TC-5-E-IST — Config E, Preset Date, IST: Bug #7 stores Feb 28 instead of Mar 1; legacy does not protect preset path
...
- Interpretation: V2 is active instead of V1. This test documents V1 behavior. V2 partially fixes Bug #7 for non-ignoreTZ fields (Config E has `ignoreTimezone=false`), so V2 may produce different results for this config.
...
- Interpretation: Bug #7 confirmed on the preset/form-init path for Config E. `initCalendarValueV1` uses hardcoded `parseDateString(initialDate, false, true)` → `moment(stripped).startOf('day').toDate()` → IST local midnight (March 1 00:00 IST = Feb 28 18:30 UTC). `useLegacy=true` does not protect against Bug #7 — the `parseDateString` path is shared between legacy and non-legacy. Same behavior as non-legacy Config A (5-A-IST).
...
| Reference               | Location                                                                               |
| ----------------------- | -------------------------------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `5-E-IST`                                                         |
| Summary                 | [summary](../summaries/tc-5-E-IST.md)                                                  |
| Bug #7 analysis         | `../analysis.md` — Bug #7 (date-only SetFieldValue wrong day in UTC+)                  |
| Sibling: 5-E-BRT (PASS) | [tc-5-E-BRT.md](tc-5-E-BRT.md) — same config, no bug in BRT                            |
| Sibling: 5-A-IST (FAIL) | [tc-5-A-IST.md](tc-5-A-IST.md) — Config A same Bug #7 behavior (non-legacy equivalent) |
| Sibling: 5-F-IST (FAIL) | [tc-5-F-IST.md](tc-5-F-IST.md) — Config F same Bug #7 behavior (ignoreTZ variant)      |
