# TC-7-A-usFormatTime — Config A, SetFieldValue US format+time, BRT: time stripped, stored as-is; no bugs in UTC-
...
| **Scenario**            | `2026-03-15`, BRT — US format with time `MM/dd/yyyy hh:mm:ss a`, date-only field strips time |
...
| 2   | Set value via US format with time (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '03/15/2026 12:00:00 AM')` | Field displays `03/15/2026`                        | ☐   |
...
> US format with time (`"03/15/2026 12:00:00 AM"`) is parsed as local midnight by `moment()`. The time component is stripped by `getSaveValue()` for date-only Config A. Same storage result as the plain US format sibling.
...
| Reference              | Location                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-A-usFormatTime`                                                 |
| Results index          | `../results.md` § Session 2026-04-03                                                    |
| Bug #7 analysis        | `../analysis.md` — Bug #7 absent in BRT (UTC-), fires in UTC+ only                      |
| US format sibling      | [`tc-7-A-usFormat.md`](tc-7-A-usFormat.md) — same format without time component         |
