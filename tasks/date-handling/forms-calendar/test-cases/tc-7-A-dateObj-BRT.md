# TC-7-A-dateObj-BRT — Config A, SetFieldValue Date object, BRT: stored as-is; no bugs in UTC- timezone
...
> Date object input (`new Date(2026, 2, 15)`) creates local midnight. In BRT (UTC-3), `normalizeCalValue()` calls `moment(Date).toDate()` which preserves the local date. `getSaveValue()` extracts the date portion correctly. In UTC+ timezones (IST), this same input causes a double-shift — see `tc-7-A-dateObj-IST.md`.
...
| Reference              | Location                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-A-dateObj-BRT`                                                  |
| Results index          | `../results.md` § Session 2026-04-03                                                    |
| Bug #7 analysis        | `../analysis.md` — Bug #7 absent in BRT (UTC-), fires in UTC+ only                      |
| IST sibling            | [`tc-7-A-dateObj-IST.md`](tc-7-A-dateObj-IST.md) — Bug #7 double-shift, stores -2 days  |
