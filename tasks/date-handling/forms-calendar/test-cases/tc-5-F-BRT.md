# TC-5-F-BRT — Config F, Preset Date, BRT: legacy date-only + ignoreTZ stores correctly; GFV returns raw Date (no bugs)
...
> **Note on ignoreTZ and legacy preset behavior**: `ignoreTimezone=true` has no effect on the preset path — Bug #3 hardcodes it anyway. Legacy date-only preset behavior is identical to non-legacy (Config B). `parseDateString` truncates to midnight, and neither the legacy flag nor the ignoreTZ flag alters the date-only init path. Legacy GFV returns the raw value unchanged — no Bug #5 fake Z transformation. For BRT (UTC-3), local midnight March 1 = UTC 03:00 March 1, so save extraction produces the correct `"2026-03-01"`.
...
- Interpretation: The test form does not have a Config F preset field, or the field configuration has changed. Verify the form template is correct.

## Related

| Reference               | Location                                                         |
| ----------------------- | ---------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `5-F-BRT`                                   |
| Summary                 | [summary](../summaries/tc-5-F-BRT.md)                            |
| Sibling: 5-F-IST (FAIL) | matrix.md — row `5-F-IST` (Bug #7 on same config in UTC+)        |
| Sibling: 5-B-BRT (PASS) | [tc-5-B-BRT.md](tc-5-B-BRT.md) — non-legacy equivalent, PASS     |
