# TC-7-C-usFormatTime — Config C, SetFieldValue US+Time, BRT: midnight stored as local; GFV returns real UTC
...
| 2   | Set value via US format with time (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '03/15/2026 12:00:00 AM')` | Field displays `03/15/2026 12:00 AM`                      | ☐   |
...
> US format with explicit time `"03/15/2026 12:00:00 AM"` is parsed as local BRT midnight. The explicit midnight time component makes no difference compared to the time-less US format input — both resolve to the same stored value. Config C GFV correctly reconstructs UTC.
...
| Reference              | Location                                                                               |
| ---------------------- | -------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-usFormatTime`                                                |
