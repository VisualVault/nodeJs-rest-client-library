# TC-7-B-dateOnly-BRT — Config B, SetFieldValue date string, BRT: stored as-is; ignoreTZ inert on date-only
...
| **Scenario**            | `2026-03-15`, BRT — date-only field with `ignoreTimezone=true`                           |
...
> Config B (`ignoreTimezone=true`) behaves identically to Config A for date-only fields — the `ignoreTimezone` flag has no effect when `enableTime=false`. This test confirms that equivalence in BRT.
...
| Reference              | Location                                                                                      |
| ---------------------- | --------------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-B-dateOnly-BRT`                                                       |
| Results index          | `../results.md` § Session 2026-04-03                                                          |
| Bug #7 analysis        | `../analysis.md` — Bug #7 absent in BRT (UTC-), fires in UTC+ only                            |
| Config A sibling       | [`tc-7-A-dateOnly.md`](tc-7-A-dateOnly.md) — identical behavior (ignoreTZ inert on date-only) |
| IST sibling            | [`tc-7-B-dateOnly-IST.md`](tc-7-B-dateOnly-IST.md) — Bug #7 fires, stores previous day        |
