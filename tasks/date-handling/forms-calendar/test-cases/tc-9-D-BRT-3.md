# TC-9-D-BRT-3 — Config D, Round-Trip 3 trips, BRT: -9h cumulative drift (Bug #5)
...
| **Scenario**            | `2026-03-15` baseline, 3 GFV round-trips in BRT — each trip drifts -3h (Bug #5)         |
...
| 4   | Execute 3 GFV round-trips in sequence (console) | `for (var i=0; i<3; i++) { VV.Form.SetFieldValue('<FIELD_NAME>', VV.Form.GetFieldValue('<FIELD_NAME>')); }` | No error                                                 | ☐   |
...
**FAIL-1 (Bug #5 cumulative drift — -9h after 3 trips):**
Step 5 returns `"2026-03-14T15:00:00"` instead of `"2026-03-15T00:00:00"`.

- Interpretation: Bug #5 caused -3h per trip × 3 trips = -9h. GFV adds fake Z → SFV parses as UTC midnight → stores BRT local (UTC-3). Trip progression: `-3h → -6h → -9h`. The time drifted to 3:00 PM on March 14.
...
| Reference                 | Location                                                         |
| ------------------------- | ---------------------------------------------------------------- |
| Matrix row                | `../matrix.md` — row `9-D-BRT-3`                                 |
| Run file                  | [`../runs/tc-9-D-BRT-3-run-1.md`](../runs/tc-9-D-BRT-3-run-1.md) |
| Summary                   | [`../summaries/tc-9-D-BRT-3.md`](../summaries/tc-9-D-BRT-3.md)   |
...
| Full day lost: 9-D-BRT-8  | [`tc-9-D-BRT-8.md`](tc-9-D-BRT-8.md) — -24h after 8 trips        |
| IST comparison: 9-D-IST-8 | [`tc-9-D-IST-8.md`](tc-9-D-IST-8.md) — +44h (opposite direction) |
