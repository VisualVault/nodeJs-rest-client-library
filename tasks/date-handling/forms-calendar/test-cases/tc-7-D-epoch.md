# TC-7-D-epoch — Config D, SetFieldValue Unix epoch, BRT: epoch->Date->local midnight stored; GFV adds fake Z (Bug #5)
...
| 2   | Set value via Unix epoch ms (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', 1773543600000)`         | Field displays `03/15/2026 12:00 AM`                   | ☐   |
...
> `1773543600000` is the Unix epoch for `2026-03-15T03:00:00.000Z` = BRT midnight (`2026-03-15T00:00:00-03:00`). The number input is first converted to a Date object by `normalizeCalValue()`, then processed identically to the Date object path.
...
- Interpretation: Bug #5 active. GFV appends fake Z. Storage (Step 3) is correct.
...
| Reference               | Location                                                                              |
| ----------------------- | ------------------------------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `7-D-epoch`                                                      |
