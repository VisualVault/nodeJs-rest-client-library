# TC-7-C-usFormat — Config C, SetFieldValue US Format, BRT: midnight stored as local; GFV returns real UTC
...
| 2   | Set value via US format string (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '03/15/2026')`          | Field displays `03/15/2026 12:00 AM`                      | ☐   |
...
> US format `"03/15/2026"` is parsed as local BRT midnight via `moment("03/15/2026").toDate()`. Config C stores local time and GFV reconstructs real UTC. Behavior is identical to date-only string and ISO-without-Z inputs.
...
| Reference              | Location                                                                               |
| ---------------------- | -------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-usFormat`                                                    |
