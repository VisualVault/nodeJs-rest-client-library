# TC-3-B-BRT-BRT — Config B, Server Reload, BRT: date-only string survives save/reload; GFV unchanged
...
- Interpretation: `getCalendarFieldValue()` is transforming the stored value on output. For Config B (`enableTime=false`), Bug #5 (fake Z) should not apply. If GFV returns a different format, investigate `getCalendarFieldValue()` for unexpected date-only transformations.

## Related

| Reference                  | Location                                                              |
| -------------------------- | --------------------------------------------------------------------- |
| Matrix row                 | `../matrix.md` — row `3-B-BRT-BRT`                                    |
| Run history                | [summary](../summaries/tc-3-B-BRT-BRT.md)                             |
| Bug #7 analysis            | `../analysis.md` — Bug #7 (Date-only SetFieldValue wrong day in UTC+) |
| Sibling: Config A same-TZ  | [tc-3-A-BRT-BRT.md](tc-3-A-BRT-BRT.md)                                |
| Sibling: Config D same-TZ  | [tc-3-D-BRT-BRT.md](tc-3-D-BRT-BRT.md)                                |
| Sibling: cross-TZ Config B | `../matrix.md` — row `3-B-BRT-IST` (PENDING)                          |
