# TC-8B-A-BRT — Config A, GetDateObject, BRT: correct Date at local midnight; toISOString returns real UTC
...
> Date-only Config A: GDOC creates a Date at local BRT midnight from `"2026-03-15"`. The Date's `.toISOString()` returns real UTC (+3h). Identical behavior expected for Config E (legacy date-only).
...
| Reference              | Location                                                |
| ---------------------- | ------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `8B-A-BRT`                         |
| Analysis               | `../analysis.md` — no bugs for date-only GDOC in BRT    |
| Sibling (IST)          | [`tc-8B-A-IST.md`](tc-8B-A-IST.md) — Bug #7 upstream    |
| Sibling (UTC0)         | [`tc-8B-A-UTC0.md`](tc-8B-A-UTC0.md)                    |
| Config E (legacy)      | [`tc-8B-E-BRT.md`](tc-8B-E-BRT.md) — identical behavior |
