# TC-9-B-any — Summary

**Spec**: [tc-9-B-any.md](../test-cases/tc-9-B-any.md)
**Current status**: PASS — last run 2026-04-09 (BRT, Chromium)
**Bug surface**: none — Config B round-trip stable (date-only + ignoreTZ)

## Run History

| Run | Date       | TZ  | Outcome | Notes                                      |
| --- | ---------- | --- | ------- | ------------------------------------------ |
| L2  | 2026-04-02 | —   | PASS    | Playwright Layer 2 regression verification |
| 1   | 2026-04-03 | BRT | PASS    | [run-1](../runs/tc-9-B-any-run-1.md)       |
| 2   | 2026-04-09 | BRT | PASS    | [run-2](../runs/tc-9-B-any-run-2.md)       |

## Current Interpretation

Run 2 (2026-04-09, Chromium): PASS. Cross-browser verification in progress.

## Next Action

No further action — behavior characterized.
