/**
 * LibLTAStep1GenerateFeeSummary
 * Category: Workflow
 * Modified: 2026-03-06T13:29:33.607Z by mauro.rapuano@visualvault.com
 * Script ID: Script Id: 23533acf-c01b-f011-82d6-afeb582902bd
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
    Script Name:    LibLTAStep1GenerateFeeSummary
    Customer:       WADNR
    Purpose:        The purpose of this library is to generate and automatically assess fees for the LTA Step 1

    Preconditions:  None
    Parameters:
                    LTA Step 1 Form ID (String, Required)
    ...
    Pseudo code:
                    ...
                    3 Get the LTA Step 1 Form and its fields
    ...
    Date of Dev:    04/17/2025
    Last Rev Date:  03/05/2026

    Revision Notes:
                    04/17/2025 - Alfredo Scilabra:  First Setup of the script
                    06/03/2025 - Fernando Chamorro: Adding UserID param to fix Dashboard filter
                    08/15/2025 - Moises Savelli:    Adding more error handling about the individual ID
    ...
    */
    ...
    logger.info(`Start of the process LibLTAStep1GenerateFeeSummary  at ${Date()}`);
    ...
    const CONTIGUOS_FEE_LOOKUP_NAME = 'FPAN Application - Contiguous activities for Small Forest Landowner';
    const NON_CONTIGUOS_FEE_LOOKUP_NAME = 'FPAN Application - Non-contiguous activities for Small Forest Landowner';
    ...
    function getLTAStepOneFormRecord(ltaStepOneFormID) {
        return getFormsRecords(
            {
                q: `[instanceName] eq '${ltaStepOneFormID}'`,
                expand: true,
            },
    ...
    function getRelatedFees(ltaStepOneFormID) {
        return getFormsRecords(
            {
                q: `[Related Record ID] eq '${ltaStepOneFormID}' AND ([Status] eq 'Unpaid' OR [Status] eq 'Paid')`,
                expand: true,
            },
            'Fee'
        );
    ...
    function getCalculatedFee(ltaStepOneFormRecord, contiguousFee, nonContiguousFee) {
        // Question 2 Are you a small forest landowner per RCW 76.09.450?
        const isSmallForestLandowner = ltaStepOneFormRecord['small Forest Landowner'] === 'Yes';
        // Is proposed harvest area is on a single contiguous ownership consisting of one or more parcels
        const isharvestAreaContiguous = parseBool(ltaStepOneFormRecord['harvest Area Contiguous']);

        if (!isSmallForestLandowner) return { 'fee Amount': 0 };
        return isharvestAreaContiguous ? contiguousFee : nonContiguousFee;
    }

    function createFee(ltaStepOneFormRecord, feeLookup, feeAmount, userID) {
        const formRelatedFields = {
            'Individual ID': ltaStepOneFormRecord['individual ID'] || ltaStepOneFormRecord['Individual ID'],
            'Related Record ID': ltaStepOneFormRecord.instanceName,
    ...
    try {
        // 1
        const ltaStepOneFormID = getFieldValueByName('LTA Step 1 Form ID');
        ...
        if (!ltaStepOneFormID || !userID) {
            // Throw every error getting field values as one concatenated string
            throw new Error(errorLog.join('; '));
        }

        // 3 Get the LTA Step 1 Form and its fields
        const [ltaStepOneFormRecord] = await getLTAStepOneFormRecord(ltaStepOneFormID);

        if (isNullEmptyUndefined(ltaStepOneFormRecord)) {
            throw new Error('Step 1 Long Term FPA record not found');
        }

        // 4 Get the Fee Lookup Records list.
        const [[contiguousFeeLookupRecord], [nonContiguousFeeLookupRecord]] = await Promise.all(
            [CONTIGUOS_FEE_LOOKUP_NAME, NON_CONTIGUOS_FEE_LOOKUP_NAME].map((feeName) => getFeeLookupRecord(feeName))
        );

        const lookupRecordsValid = areFeeLookupRecordsValid(contiguousFeeLookupRecord, nonContiguousFeeLookupRecord);
        if (!lookupRecordsValid) throw new Error('Fee Lookup record missing');

        // 5 Get fees associated with the application and total amount of the fees.
        const relatedFees = await getRelatedFees(ltaStepOneFormID);

        // 6 Identify all the possible combinations of fees based on questions answered.
        const calculatedFee = getCalculatedFee(
            ltaStepOneFormRecord,
        ...
        if (calculatedFee['fee Amount'] !== 0 && !noPaymentRequired) {
            ...
            if (relatedPaidFees.length == relatedFees.length) {
                await createFee(ltaStepOneFormRecord, calculatedFee, differenceBetweenFees.value, userID);
