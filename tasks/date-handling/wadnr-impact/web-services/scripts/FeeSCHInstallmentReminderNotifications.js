...
module.exports.main = async function (vvClient, response, token) {
  ...
  */
    ...
    const ScheduledProcessName = 'FeeSCHInstallmentReminderNotifications';
  ...
  *****************/
    ...
    const UpcomingDueInstallmentQueryName = 'zWebSvc Upcoming Due Instl Fees Need Notification';
    const OverdueInstallmentQueryName = 'zWebSvc Overdue Installment Fees Need Notification';
    const UpcomingDueInstallmentEmailTemplateName = 'Upcoming Due Installment Fee for Profile Administrators';
    const OverdueInstallmentEmailTemplateName = 'Overdue Installment Fee for Profile Administrators';
    const FeeNameEmailToken = '[Fee Name]';
    const NextDueDateEmailToken = '[Next Due Date]';
    const UpcomingDueInstallmentNotificationCompleteCheckboxName = 'Upcoming Due Installment Fee Notification Sent';
    const OverdueInstallmentNotificationLastSendDateFieldName = 'Overdue Installment Fee Notification Last Send Date';

    /* Admin Email Constants */
    const AdministratorGroups = ['VaultAccess'];
    const ScheduledProcessNameEmailToken = '[Scheduled Process Name]';
    const HardErrorFoundAdminImmediateEmailTemplateName = 'Hard Error Encountered in Scheduled Process';
    const SoftErrorsFoundAdminDigestEmailTemplateName = 'Soft Errors Encountered in Scheduled Process';
  ...
  *****************/
    ...
    *****************/
        ...
        async function UpcomingDueInstallmentNotification() {
            ...
            let feesThatNeedNotification = customQueryResp.hasOwnProperty('data') ? customQueryResp.data : null;
            ...
            for (const fee of feesThatNeedNotification) {
                try {
                    ...
                    if (postFormResp.meta.status !== 201) {
                        ...
                        throw new Error(`Unable to update the ${FeeTemplateID} form. ${message}.`);
                    }
                } catch (error) {
                    // any errors processing individual fees can be considered soft since one fee failing doesn't fail all fees
                    SoftErrorLog.push(`Error processing Fee ${fee['dhDocID']}: ${error.message}`);
                    feeErrorCount++;
                }
            }

            if (feeErrorCount >= feesThatNeedNotification.length) {
                throw new Error('Errors encountered processing all fees in a single iteration.');
            }
        }
        ...
        async function getProviderProfileAdminEmails(providerID) {
            ...
            if (getFormsResp.meta.status !== 200) {
                throw new Error(`There was an error when calling getForms on ${StaffRelationTemplateID}.`);
            }
            if (Array.isArray(getFormsData) === false) {
                throw new Error(
                    `Data was not able to be returned when calling getForms on ${StaffRelationTemplateID}.`
                );
            }
            if (getFormsData.length < 1) {
                throw new Error(`Unable to find profile administrators for provider ${providerID}.`);
            }
            ...
            if (getFormsResp.meta.status !== 200) {
                throw new Error(`There was an error when calling getForms on ${IndividualRecordTemplateID}.`);
            }
            if (Array.isArray(getFormsData) === false) {
                throw new Error(
                    `Data was not able to be returned when calling getForms on ${IndividualRecordTemplateID}.`
                );
            }
            if (getFormsData.length < 1) {
                throw new Error(`Unable to find emails for the profile administrators of provider ${providerID}.`);
            }
        ...
        async function createUpcomingDueInstallmentEmail(fee, recipientEmails) {
            ...
            let generateEmailResp = await vvClient.scripts.runWebService(GenerateEmailWebsvcName, emailRequestArr);
            let generateEmailData = generateEmailResp.hasOwnProperty('data') ? generateEmailResp.data : null;
            if (generateEmailResp.meta.status !== 200) {
                throw new Error(`There was an error when calling ${GenerateEmailWebsvcName}.`);
            } else if (!Array.isArray(generateEmailData)) {
                throw new Error(`Data was not returned when calling ${GenerateEmailWebsvcName}.`);
                ...
                throw new Error(`The call to ${GenerateEmailWebsvcName} returned with an unhandled error.`);
            }
        }
        ...
        async function createProcessErrorsAdminEmail() {
            ...
            if (SoftErrorLog.length > MaxErrorsToLogInEmail) {
                ...
                emailSoftErrorLog.push(`And ${SoftErrorLog.length - MaxErrorsToLogInEmail} additional process errors`);
            }

            // determine email template & tokens
            let tokenArr = [{ name: ScheduledProcessNameEmailToken, value: ScheduledProcessName }];
            ...
            if (HardErrorLog.length > 0) {
                ...
                let errorMessageHTML = `${buildUnorderedListHTMLFromArr(emailSoftErrorLog)}`;
        ...
        async function getUserGroupMemberEmailsNoMap(groupNames) {
            ...
            let groupEmailAddressesData = groupEmailAddressesResp.hasOwnProperty('data')
                ? groupEmailAddressesResp.data
                : null;
            if (groupEmailAddressesResp.meta.status !== 200) {
                throw new Error(`There was an error when calling ${GetGroupUserEmailsWebsvcName}.`);
            } else if (!Array.isArray(groupEmailAddressesData)) {
                throw new Error(`Data was not returned when calling ${GetGroupUserEmailsWebsvcName}.`);
                ...
                throw new Error(`The call to ${GetGroupUserEmailsWebsvcName} returned with an unhandled error.`);
            }
