/**
 * LibMoveUploadedFilesToFinalPath
 * Category: Workflow
 * Modified: 2026-03-20T19:33:53.23Z by john.sevilla@visualvault.com
 * Script ID: Script Id: d2a4580d-82ae-f011-82f2-ac04d8d0fca0
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
  Script Name:    LibMoveUploadedFilesToFinalPath
  Customer:       WADNR
  ...
  */
    ...
    logger.info(`Start of the process LibMoveUploadedFilesToFinalPath at ${Date()}`);
    ...
    function getPathFromFormNumber(formFolder, applicationNumber, includesRange, iniFolder, isWTM = false) {
        applicationNumber = String(applicationNumber).trim();
        const includeAppsFolder = isWTM;
        let path = '';
    ...
    try {
        ...
        const folderTasks = formsFoldersToMove.map(async (formFolder) => {
            ...
            const finalPath = getPathFromFormNumber(formFolder, fpaNumber, includesRange, iniFolder, isWTM);

            // Validate final path
            if (!finalPath) {
                const formType = isAppeal ? 'Appeal' : isFpets ? 'FPETS' : isWTM ? 'WTM' : 'FPA';
                throw new Error(
