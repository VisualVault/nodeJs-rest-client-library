 ...
 * Category: Form
 * Modified: 2025-09-08T14:05:34.963Z by mauro.rapuano@visualvault.com
 * Script ID: Script Id: 818aca35-fe87-f011-82f3-e2fa72035411
...
module.exports.main = async function (ffCollection, vvClient, response) {
    /*
    Script Name:    GetWTMNumberAndDateOfReceipt
    Customer:       WADNR
    Purpose:        Gets a unique WTMF reference ID (RE-WTM-YY-NNNN) for a specified Region, and corresponding Date Of Receipt.
                    Being:  RE - Region code (2 letters)
                            WTM - Form name (3 letters)
    ...
    Parameters:     The following represent variables passed into the function:
                    ...
                    WTM Review Page Form ID: (string, required) Form ID of the WTM Review Page
    Return Object:
    ...
    Revision Notes:
                    09/08/2025 - Mauro Rapuano:  First Setup of the script
    */

    logger.info(`Start of the process GetWTMNumberAndDateOfReceipt at ${Date()}`);
    ...
    const formTemplateName = 'Water Type Modification Form';
    ...
    try {
        ...
        const wtmReviewId = getFieldValueByName('WTM Review Page Form ID');

        // 2° Check if the required parameters are present and complete list of holidays
        if (!formId || !regionCode || !wtmReviewId) {
        ...
        const uniqueNumber = wtmReviewId.slice(-4).padStart(4, '0');

        // 5° Build the Unique Reference ID based on parameters and results
        let year = dayjs(dateOfReceipt).year().toString().substring(2);
        let uniqueReferenceID = `${regionCode}-WTM-${year}-${uniqueNumber}`;

        const returnObj = {
            WTM_Number: uniqueReferenceID,
            DateOfReceipt: dateOfReceipt,
        };

        outputCollection[0] = 'Success'; // Don´t change this
        outputCollection[1] = 'WTM Number and Date generated succesfully';
