/**
 * VV.Form.Global.DateMilestoneCalculateKPI
 * Parameters: 1
 * Extracted from WADNR (vv5dev/fpOnline) on 2026-04-08
 */
function (inspectionId) {
// ScriptName: DateMilestoneCalculateKPI

const webServiceName = 'LibInspectionDateMilestoneCalculateKPI';
const formName = 'Inspection';
        ...
        if (typeof resp.status != 'undefined') {
            ...
            if (resp.data[0] != undefined) {
                if (resp.data[0] == 'Success') {
                    ...
                    const calculatedDateList = resp.data[2];

                    Promise.all(calculatedDateList.map(calculatedDate => VV.Form.SetFieldValue(calculatedDate.name, calculatedDate.value, true)))
                    .then(()=> VV.Form.DoAjaxFormSave())
                    .then(()=> VV.Form.HideLoadingPanel())
