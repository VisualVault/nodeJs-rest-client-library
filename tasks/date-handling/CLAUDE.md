...
**~112 of ~242 test cases completed** (58 PASS, 41 FAIL). 7 bugs confirmed (5 from original analysis + 2 new, Bug #6 scope expanded). Testing across three timezones: BRT (UTC-3), IST (UTC+5:30), UTC+0 (GMT). 11 Playwright spec files with 87 test-data entries covering Categories 1, 2, 3, 5, 6, 7, 8, 8B, 9, 9-GDOC, 12. **Cat 1 fully backfilled** (20/20 entries, correct expected values). **Cat 2 fully backfilled** (16/16 entries). Cat 3 (Server Reload) 11/18 (9P, 2F) — includes legacy Config G same-TZ reload. Cat 5 (Preset) 2 entries (1P, 2F — Bug #5 fires on preset Config D at form load, Bug #7 on Config A). Cat 6 (Current Date) 3 entries (2P, 1F — Bug #5 fires on current date Config D at form load). Cat 7 (SFV Formats) 16 entries across 3 configs × 2 TZs (4P, 7F api — Bug #5 fake Z on all Config D; Bug #7 -1/-2 day on Config A IST; Config C passes clean). Cat 8 (GFV Return) 6P, 5F — Bug #6 scope: Config C+D affected, Config H immune (useLegacy=true prevents Bug #6). Cat 8B (GDOC) 3 entries (3P — IST returns correct real UTC, empty returns undefined not Invalid Date). Cat 9 (GFV Round-Trip) 6P, 5F — Config H 0 drift confirmed in IST (universal protection). Cat 9-GDOC 2P (GDOC round-trip stable in BRT AND IST — matrix prediction of +5:30h was wrong). Cat 12 (Edge Cases) 5P, 5F — UTC+0 control confirms Bug #5 drift = TZ offset.
