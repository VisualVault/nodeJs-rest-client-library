...
exports.processRequest = function (req, res) {
    ...
    if (typeof req.query.id === 'string') {
        ...
        fs.writeFile(VVScriptFilePath + fileId + '.js', script, function (err) {
            if (err) {
                ...
                Q.when(
                    ...
                    .fail(function (error) {
                        logger.error(
                            'Error response from vvAuthorize.getVaultApi. http 401 response returned. ' + error.message
                        );
        ...
        try {
            ...
            logger.error(
                'http 500: Error Loading scheduled script file requested by query string: ' +
                    scriptName +
                    '.js. http 500 response returned. ' +
                    ex
            );
