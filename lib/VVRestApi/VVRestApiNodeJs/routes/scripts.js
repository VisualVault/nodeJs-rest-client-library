...
exports.scripts = function (req, res) {
    ...
    if (typeof req.query.id === 'string') {
        ...
        if (fileId == 'localScript') {
            ...
            vvAuthorize.readOnly = !!(global.VV_ENV && global.VV_ENV.readOnly);
            vvAuthorize.writePolicy = (global.VV_ENV && global.VV_ENV.writePolicy) || null;

            //call VisualVault to get access token
            Q.when(
                vvAuthorize.getVaultApi(
                    params.clientId,
                    params.clientSecret,
                    params.userId,
                    params.password,
                    params.audience,
                    params.baseUrl,
                    params.customerAlias,
                    params.databaseAlias
                )
            )
                ...
                .fail(function (error) {
                    if (
                        error.message &&
                        (error.message.indexOf('Authorization') >= 0 || error.message.indexOf('token') >= 0)
                    ) {
            ...
            fs.writeFile(VVScriptFilePath + fileId + '.js', script, function (err) {
                if (err) {
                    ...
                    logger.error(
                        params.baseUrl + ' Unable to write script file. http 500 response returned. ' + error.message
                    );
                } else {
                    ...
                    Q.when(
                        ...
                        .fail(function (error) {
                            if (
                                ...
                                logger.error(
                                    params.baseUrl +
                                        ' Error acquiring access token. http 401 response returned. ' +
                                        error.message
                                );
                            } else {
                                //error while executing script
                                ...
                                logger.error(
                                    params.baseUrl +
                                        ' Error acquiring access token. http 500 response returned. ' +
                                        error.message
                                );
                            }
