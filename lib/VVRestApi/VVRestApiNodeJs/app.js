// Load hierarchical credentials from .env.json (gitignored)
// Structure: servers.{name}.customers.{name} — selected via activeServer + activeCustomer
var envJsonPath = require('path').resolve(__dirname, '../../../.env.json');
try {
    var envConfig = JSON.parse(require('fs').readFileSync(envJsonPath, 'utf8'));
    var server = envConfig.servers && envConfig.servers[envConfig.activeServer];
    var customer = server && server.customers && server.customers[envConfig.activeCustomer];
    if (server && customer) {
        global.VV_ENV = Object.assign({}, customer, { baseUrl: server.baseUrl });
        if (customer.readOnly) {
            console.warn(
                '[READ-ONLY] ' +
                    envConfig.activeServer +
                    '/' +
                    envConfig.activeCustomer +
                    ' is marked readOnly. Write operations will be blocked.'
            );
        }
    }
} catch (e) {
