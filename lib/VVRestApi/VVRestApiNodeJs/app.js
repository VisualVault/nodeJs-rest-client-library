// Load multi-environment credentials from .env.json (gitignored)
var envJsonPath = require('path').resolve(__dirname, '../../../.env.json');
try {
    var envConfig = JSON.parse(require('fs').readFileSync(envJsonPath, 'utf8'));
    var activeEnv = envConfig.environments[envConfig.activeEnv];
    if (activeEnv) global.VV_ENV = activeEnv;
} catch (e) {
