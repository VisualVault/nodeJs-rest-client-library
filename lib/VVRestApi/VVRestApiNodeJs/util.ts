 ...
 *
 * Alterations have been made to the original AWS Util file to make it compatable with VisualVault
 */
...
/**
 * A set of utility methods for use with the VV SDK.
 *
 * @!attribute abort
 *   Return this value from an iterator function ({each} or {arrayEach})
 *   to break out of the iteration.
 *   @example Breaking out of an iterator function
 *     VV.util.each({a: 1, b: 2, c: 3}, function(key, value) {
 *       if (key == 'b') return VV.util.abort;
 *     });
 *   @see each
 *   @see arrayEach
 * @api private
 */
class  vaultUtilBase64Helper{

    _vaultUtil : VaultUtility;

    constructor(vaultUtil : VaultUtility) {
...
class VaultUtility {
 ...
 constructor() {
  ...
  queryParamsToString(params) {
    ...
    VV.util.arrayEach(sortedKeys, function(name) {
      ...
      if (Array.isArray(value)) {
        var vals = [];
        VV.util.arrayEach(value, function(item) { vals.push(escape(item)); });
  ...
  base64 : vaultUtilBase64Helper;

  buffer: vaultUtilBufferHelper;

  string: vaultUtilStringHelper;

  /**
   * Date and time utility functions.
   */
  date: vaultUtilDateHelper;

  crypto: vaultUtilCryptoHelper;
