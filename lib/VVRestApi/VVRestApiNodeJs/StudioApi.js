...
module.exports = class StudioApi {
    constructor(sessionToken, studioApiConfig, options) {
        ...
        this._httpHelper = new common.httpHelper(sessionToken, yamlConfig, options);
...
class WorkflowManager {
    ...
    async getWorkflow(workflowId) {
        var resourceUri = this._httpHelper._config.ResourceUri.StudioApi.WorkflowLatestPublishedId.replace(
            '{id}',
            workflowId
        );
    ...
    async triggerWorkflow(workflowId, workflowRevision, objectId, workflowVariables) {
        var resourceUri = this._httpHelper._config.ResourceUri.StudioApi.WorkflowRun.replace(
            '{id}',
            workflowId
    ...
    async terminateWorkflow(workflowId, instanceId) {
        var resourceUri = this._httpHelper._config.ResourceUri.StudioApi.WorkflowTerminate.replace(
            '{workflowId}',
            workflowId
    ...
    async GetWorkflowHistoryForObject(objectId, workflowId) {
        var resourceUri = this._httpHelper._config.ResourceUri.StudioApi.WorkflowHistoryObject.replace(
            '{workflowId}',
            workflowId
    ...
    async GetRunningWorkflowForObject(objectId, workflowId) {
        var resourceUri = this._httpHelper._config.ResourceUri.StudioApi.WorkflowHistoryRunningObject.replace(
            '{workflowId}',
            workflowId
        ).replace('{objectId}', objectId);
        var url = this._httpHelper.getUrl(resourceUri);
