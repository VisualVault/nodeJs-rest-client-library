...
(function (vvRestApi) {
    ...
    var authorize = (function () {
        ...
        authorize.prototype.acquireSecurityToken = function (loginToken, developerId, developerSecret, baseUrl, apiUrl, customerAlias, databaseAlias, authenticationUrl) {
            ...
            this.Q
                .when(
                    this.__getToken(loginToken, developerId, developerSecret, baseUrl, customerAlias, databaseAlias, authenticationUrl, true)
                )
    ...
    (function (common) {
        ...
        var jwt = (function () {
            ...
            jwt.prototype.encode = function (payload, key, algo) {
                ...
                algo = algo || 'HS256';
                var header = { typ: 'JWT', alg: algo };

                //console.log('url safe B64 of headers ' + this.urlsafeB64Encode(JSON.stringify(header)));
                //console.log('url safe B64 of payload ' + this.urlsafeB64Encode(JSON.stringify(payload)));

                var segments = [this.urlsafeB64Encode(JSON.stringify(header)), this.urlsafeB64Encode(JSON.stringify(payload))];

                var signing_input = segments.join('.');
                var signature = this.sign2(signing_input, key, algo);

                segments.push(this.urlsafeB64Encode(signature));

                //console.log('segments has ' + segments.length + ' elements in its array');

                var jwtToken = segments.join('.');
            ...
            jwt.prototype.sign2 = function (msg, key, method) {
                method = method || 'HS256';
                var methods = {
                    HS256: 'sha256',
                    HS512: 'sha512'
                ...
                var hmac = this.crypto.createHmac(methods[method], key);
        ...
        var requestSigning = (function () {
            ...
            requestSigning.prototype.sortQueryStringParamObject = function (obj, prefix, uriEncode) {
                var str = [];
                for (var p in obj) {
