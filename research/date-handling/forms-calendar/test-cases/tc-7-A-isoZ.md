# TC-7-A-isoZ — Config A, SetFieldValue ISO+Z, BRT: time+Z stripped, date extracted correctly; no bugs in UTC-
...
| **Scenario**            | `2026-03-15`, BRT — date-only field, ISO+Z input with time component                      |
...
| 2   | Set value via ISO+Z string (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '2026-03-15T00:00:00.000Z')` | Field displays `03/15/2026`                        | ☐   |
| 3   | Capture raw stored value (DevTools console)   | `VV.Form.VV.FormPartition.getValueObjectValue('<FIELD_NAME>')`      | `"2026-03-15"`                                     | ☐   |
| 4   | Capture GetFieldValue (DevTools console)      | `VV.Form.GetFieldValue('<FIELD_NAME>')`                             | `"2026-03-15"` — date string, time+Z stripped      | ☐   |
...
> Config A (`enableTime=false`) strips the time and Z from ISO input, extracting just the date portion. In BRT (UTC-), this produces the correct date. In UTC+ timezones, Bug #7 causes a -1 day shift because the date extraction happens after local time conversion.
...
| Reference                 | Location                                                                                   |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| Matrix row                | `../matrix.md` — row `7-A-isoZ`                                                            |
| Bug #7 analysis           | `../analysis.md` — Bug #7 absent in BRT (UTC-), fires in UTC+ only                         |
| Date-only sibling         | [`tc-7-A-dateOnly.md`](tc-7-A-dateOnly.md) — same Config A with plain date string input    |
| IST siblings              | `tc-7-A-dateOnly-IST.md`, `tc-7-A-dateObj-IST.md` — Bug #7 fires in UTC+                   |
| Config D ISO+Z comparison | [`tc-7-D-isoZ.md`](tc-7-D-isoZ.md) — DateTime Config D with ISO+Z input                    |
