# TC-7-C-dateOnly — Config C, SetFieldValue Date String, BRT: midnight stored as local; GFV returns real UTC
...
| 2   | Set value via date-only string (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '2026-03-15')`          | Field displays `03/15/2026 12:00 AM`                      | ☐   |
...
> Date-only string `"2026-03-15"` is parsed as local BRT midnight via `moment("2026-03-15").toDate()`. Config C (`ignoreTimezone=false`) stores local time and GFV correctly reconstructs UTC. Behavior is identical to ISO-without-Z input (tc-7-C-isoNoZ).
...
- Interpretation: Config C's UTC reconstruction is broken. Expected: GFV converts local stored value back to UTC. If GFV returns `"2026-03-15T00:00:00.000Z"`, it is treating the local value as UTC (fake Z issue similar to Bug #5 but on Config C).
...
| Reference              | Location                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-dateOnly`                                                       |
...
| Sibling (Date object)  | [`tc-7-C-dateObj.md`](tc-7-C-dateObj.md)                                                  |
