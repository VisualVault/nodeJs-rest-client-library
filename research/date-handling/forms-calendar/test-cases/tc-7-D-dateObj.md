# TC-7-D-dateObj — Config D, SetFieldValue Date Object, BRT: local midnight stored correctly; GFV adds fake Z (Bug #5)
...
- Interpretation: Bug #5 active. `getCalendarFieldValue()` appends literal `[Z]` via `moment(value).format("YYYY-MM-DD[T]HH:mm:ss.SSS[Z]")`. The Z is fake — it labels BRT local midnight as UTC. Storage (Step 3) is correct; only the API output is affected.
...
| Reference               | Location                                                                                 |
| ----------------------- | ---------------------------------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `7-D-dateObj`                                                       |
