# TC-8-G-BRT — Config G, GetFieldValue, BRT: legacy DateTime returns raw unchanged; no UTC conversion
...
> Config G (`useLegacy=true`, `ignoreTimezone=false`) skips the non-legacy GFV branch entirely. Unlike Config C (same flags except `useLegacy=false`) which returns real UTC `"2026-03-15T03:00:00.000Z"`, Config G returns the raw stored value unchanged. The `useLegacy=true` flag prevents both Bug #5 (fake Z) and the legitimate UTC conversion. Config G behaves identically to Config H for GFV — both legacy DateTime configs return raw.

---

## Fail Conditions

**FAIL-1 (GFV returns UTC-converted value):**
Step 4 returns `"2026-03-15T03:00:00.000Z"` instead of `"2026-03-15T00:00:00"`.

- Interpretation: Legacy DateTime GFV is applying UTC conversion. This would mean `useLegacy=true` does not skip the non-legacy branch — contradicts 8-H-BRT results and Cat 7 Config G findings.
...
| Reference              | Location                                                                                                    |
| ---------------------- | ----------------------------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `8-G-BRT`                                                                              |
| Analysis               | `../analysis.md` — useLegacy=true bypasses getCalendarFieldValue non-legacy branch                          |
| Config C comparison    | [`tc-8-C-BRT.md`](tc-8-C-BRT.md) — same flags except useLegacy=false; returns real UTC                      |
| Config H sibling       | [`tc-8-H-BRT.md`](tc-8-H-BRT.md) — legacy DateTime + ignoreTZ, also returns raw                             |
| Field config reference | `research/date-handling/CLAUDE.md` — Config G (`enableTime=true`, `ignoreTimezone=false`, `useLegacy=true`) |
