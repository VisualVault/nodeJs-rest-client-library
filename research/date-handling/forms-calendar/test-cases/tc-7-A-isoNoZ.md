# TC-7-A-isoNoZ — Config A, SetFieldValue ISO without Z, BRT: stored as-is; no bugs in UTC- timezone
...
| **Scenario**            | `2026-03-15`, BRT — ISO string without Z suffix, date-only field strips time component    |
...
| 2   | Set value via ISO string without Z (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', '2026-03-15T00:00:00')` | Field displays `03/15/2026`                        | ☐   |
...
> ISO without Z (`"2026-03-15T00:00:00"`) is parsed as local time by `moment()`. In BRT, local midnight March 15 → `getSaveValue()` extracts `"2026-03-15"`. The time component is stripped for date-only Config A fields.
...
| Reference              | Location                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------ |
| Matrix row             | `../matrix.md` — row `7-A-isoNoZ`                                                          |
