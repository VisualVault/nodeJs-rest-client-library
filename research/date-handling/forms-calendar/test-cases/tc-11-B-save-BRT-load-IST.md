# TC-11-B-save-BRT-load-IST — Config B, Cross-TZ Reload, BRT→IST: raw preserved; ignoreTZ no effect on date-only load
...
**P4 — Open the BRT-saved record** (DateTest-000080, Config B):
...
> This record was saved from BRT (UTC-3) with Config B field set to `03/15/2026`. The stored value is `"2026-03-15"`. This test verifies the date-only string survives cross-TZ reload even with `ignoreTimezone=true`.
...
- Interpretation: `initCalendarValueV1` re-parsed the stored date-only string as IST local midnight, triggering FORM-BUG-7. The `ignoreTimezone=true` flag does not protect date-only fields from the midnight parsing issue.
...
- Interpretation: DateTest form does not have a field with Config B flags.
...
| Reference                  | Location                                                                    |
| -------------------------- | --------------------------------------------------------------------------- |
| Matrix row                 | `matrix.md` — row `11-B-save-BRT-load-IST`                                  |
| Config A sibling           | [tc-11-A-save-BRT-load-IST.md](tc-11-A-save-BRT-load-IST.md) — same pattern |
| Cat 3 sibling: 3-B-BRT-IST | `matrix.md` — row `3-B-BRT-IST`                                             |
