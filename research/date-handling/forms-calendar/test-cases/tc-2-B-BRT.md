# TC-2-B-BRT — Config B, Typed Input, BRT: correct date stored; ignoreTZ inert on date-only (no bugs)
...
| 9   | Capture timezone reference                                                                                      | `` `new Date(2026, 2, 15, 0, 0, 0).toISOString()` ``                 | `"2026-03-15T03:00:00.000Z"` — confirms BRT active             | ☐   |
...
>
> **Config B vs Config A**: The `ignoreTimezone=true` flag is active on this field but has **no effect** on date-only storage. Config B and Config A store identical values for the same typed input in BRT. This test confirms that `ignoreTimezone` is inert for the date-only save path.
...
| Reference                            | Location                                                                       |
| ------------------------------------ | ------------------------------------------------------------------------------ |
| Matrix row                           | `../matrix.md` — row `2-B-BRT`                                                 |
| Run history                          | [`../summaries/tc-2-B-BRT.md`](../summaries/tc-2-B-BRT.md)                     |
| Latest run                           | [`../runs/tc-2-B-BRT-run-1.md`](../runs/tc-2-B-BRT-run-1.md)                   |
| Sibling: Config B, popup, BRT (PASS) | [`tc-1-B-BRT.md`](tc-1-B-BRT.md) — same storage, confirms Bug #2 absent        |
| Sibling: Config B, typed, IST (FAIL) | [`tc-2-B-IST.md`](tc-2-B-IST.md) — Bug #7 causes -1 day shift in IST           |
| Sibling: Config A, typed, BRT (PASS) | [`tc-2-A-BRT.md`](tc-2-A-BRT.md) — identical result confirms ignoreTZ is inert |
| Bug #7 analysis (not triggered)      | `../analysis.md` § Bug #7                                                      |
| Field config reference               | `research/date-handling/CLAUDE.md` — Test Form Fields table, Config B          |
