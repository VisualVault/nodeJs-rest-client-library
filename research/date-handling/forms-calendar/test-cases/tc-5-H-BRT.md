# TC-5-H-BRT — Config H, Preset Date, BRT: legacy DateTime + ignoreTZ stores raw Date; no Bug #5 fake Z (legacy safe)
...
| **Scenario**            | Preset DateTime `3/1/2026 08:33 AM` BRT — `2026-03-01T08:33:07-03:00` = `2026-03-01T11:33:07.735Z` UTC |
...
// Expected result includes: { name: "Field22", initialDate: "2026-03-01T11:33:07.735Z" }
...
| 2   | Verify display of preset field | Visually inspect the target field (identified in P6)           | `03/01/2026 08:33 AM` (BRT local)                                                                           | ☐   |
| 3   | Capture raw stored value       | `VV.Form.VV.FormPartition.getValueObjectValue('<FIELD_NAME>')` | Date object — `.toISOString()` = `"2026-03-01T11:33:07.735Z"`                                               | ☐   |
| 4   | Capture GFV                    | `VV.Form.GetFieldValue('<FIELD_NAME>')`                        | Date object — `.toISOString()` = `"2026-03-01T11:33:07.735Z"`                                               | ☐   |
| 5   | Verify BRT timezone active     | `new Date(2026, 2, 1, 0, 0, 0).toISOString()`                  | `"2026-03-01T03:00:00.000Z"` — confirms BRT active                                                          | ☐   |

> **Note on legacy vs non-legacy for ignoreTZ DateTime**: This is the legacy equivalent of Config D. Non-legacy Config D adds fake Z via Bug #5 (`getCalendarFieldValue`). Legacy Config H returns the raw value unchanged — Bug #5 code path requires `!useLegacy`. This is why legacy `ignoreTimezone` DateTime presets are safer than their non-legacy counterparts. The GFV output in Step 4 matches the raw value in Step 3 exactly, with no transformation applied.
...
Step 3: Raw Date `.toISOString()` does not equal `"2026-03-01T11:33:07.735Z"`.

- Interpretation: The init path transformed the preset value instead of preserving it. Verify the field configuration matches Config H and that the form template has not changed.

## Related

| Reference                      | Location                                                         |
| ------------------------------ | ---------------------------------------------------------------- |
| Matrix row                     | `../matrix.md` — row `5-H-BRT`                                   |
| Summary                        | [summary](../summaries/tc-5-H-BRT.md)                            |
| Sibling: 5-D-BRT (FAIL Bug #5) | matrix.md — row `5-D-BRT` (non-legacy equivalent, FAIL)          |
| Sibling: 5-G-BRT               | [tc-5-G-BRT.md](tc-5-G-BRT.md) — ignoreTZ=false sibling          |
