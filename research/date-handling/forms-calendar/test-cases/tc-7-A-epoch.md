# TC-7-A-epoch — Config A, SetFieldValue Unix epoch, BRT: stored as-is; no bugs in UTC- timezone
...
| **Scenario**            | `2026-03-15`, BRT — Unix epoch ms `1773543600000` = BRT midnight March 15 = `2026-03-15T03:00:00.000Z` UTC |
...
| 2   | Set value via Unix epoch (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', 1773543600000)`         | Field displays `03/15/2026`                        | ☐   |
...
> Epoch `1773543600000` = March 15, 2026 00:00:00 BRT = March 15, 2026 03:00:00 UTC. `normalizeCalValue()` creates `new Date(1773543600000)` which is local midnight March 15 in BRT. `getSaveValue()` extracts the date portion. The epoch is timezone-aware by nature, so the correct local date is always extracted.
...
| Reference              | Location                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------ |
| Matrix row             | `../matrix.md` — row `7-A-epoch`                                                           |
...
| Date object sibling    | [`tc-7-A-dateObj-BRT.md`](tc-7-A-dateObj-BRT.md) — same Config A with Date object input    |
| Config D epoch sibling | `tc-7-D-epoch.md` — DateTime config with same epoch input                                  |
