# TC-8B-C-IST — Config C, GetDateObject, IST: correct Date; toISOString returns real UTC from IST
...
| **Scenario**            | `2026-03-15T00:00:00`, IST — `2026-03-15T00:00:00+05:30` = `2026-03-14T18:30:00Z` UTC    |
...
| 8   | Compare with GetFieldValue (DevTools console)            | `VV.Form.GetFieldValue('<FIELD_NAME>')`                        | `"2026-03-14T18:30:00.000Z"` — Config C GFV returns real UTC too | ☐   |
...
> No bugs. DateTime configs are safe from Bug #7 because SetFieldValue parses the `T` separator correctly. GDOC and GFV both return real UTC for Config C.
...
Step 8 returns a value other than `"2026-03-14T18:30:00.000Z"`.
...
| Reference              | Location                                                            |
| ---------------------- | ------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `8B-C-IST`                                     |
| Analysis               | `../analysis.md` — no bugs for DateTime GDOC in IST                 |
| Sibling (BRT)          | [`tc-8B-C-BRT.md`](tc-8B-C-BRT.md) — same behavior, BRT offset      |
| GFV comparison         | [`tc-8-C-IST.md`](tc-8-C-IST.md) — Cat 8 Config C GFV (same output) |
| Field config reference | `research/date-handling/CLAUDE.md` — Config C                       |
