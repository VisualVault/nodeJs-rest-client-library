# TC-5-B-BRT — Config B, Preset Date, BRT: correct storage at local midnight; GFV returns Date object (no bugs)
...
> **Note on raw type**: Before save, preset fields store a **Date object** in the value partition, not a string. The Date object's `.toLocaleDateString()` shows the correct local date, and `.toISOString()` reflects the UTC representation. For BRT (UTC-3), local midnight March 1 = UTC 03:00 March 1 — the UTC date matches the local date, so save extraction via `toISOString().substring(0,10)` produces the correct `"2026-03-01"`.
...
- Interpretation: The test form does not have a Config B preset field, or the field configuration has changed. Verify the form template is correct.

## Related

| Reference               | Location                                                         |
| ----------------------- | ---------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `5-B-BRT`                                   |
| Summary                 | [summary](../summaries/tc-5-B-BRT.md)                            |
| Analysis — Bug #7       | `../analysis.md` — Bug #7 (absent in BRT — negative offset safe) |
| Sibling: 5-B-IST (FAIL) | [tc-5-B-IST.md](tc-5-B-IST.md) — Bug #7 on same config in UTC+   |
| Sibling: 5-A-BRT (PASS) | matrix.md — row `5-A-BRT` (same TZ, Config A)                    |
