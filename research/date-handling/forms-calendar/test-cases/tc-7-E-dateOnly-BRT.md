# TC-7-E-dateOnly-BRT — Config E, SetFieldValue date string, BRT: stored as-is; useLegacy inert on date-only
...
| **Scenario**            | `2026-03-15`, BRT — legacy date-only field with `useLegacy=true`                         |
...
> Legacy date-only (Config E, `useLegacy=true`) behaves identically to Config A for date-only fields — the `useLegacy` flag has no effect when `enableTime=false`. This test confirms that equivalence in BRT.
...
| Reference              | Location                                                                                       |
| ---------------------- | ---------------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-E-dateOnly-BRT`                                                        |
| Results index          | `../results.md` § Session 2026-04-03                                                           |
| Bug #7 analysis        | `../analysis.md` — Bug #7 absent in BRT (UTC-), fires in UTC+ only                             |
| Config A sibling       | [`tc-7-A-dateOnly.md`](tc-7-A-dateOnly.md) — identical behavior (useLegacy inert on date-only) |
| IST sibling            | [`tc-7-E-dateOnly-IST.md`](tc-7-E-dateOnly-IST.md) — Bug #7 fires, stores previous day         |
