# TC-5-B-IST — Config B, Preset Date, IST: Bug #7 stores Feb 28 instead of Mar 1; ignoreTZ=true does not protect preset path
...
// Expected result includes: { name: "Field27", initialDate: "2026-03-01T11:38:37.141Z" }
...
- Interpretation: V2 is active instead of V1. This test documents V1 behavior. V2 partially fixes Bug #7 for non-ignoreTZ fields, but Bug #7 persists in V2 for `ignoreTimezone=true` fields (Config B).
...
- Interpretation: Bug #7 confirmed on the preset/form-init path for Config B. `initCalendarValueV1` uses hardcoded `parseDateString(initialDate, false, true)` → `moment(stripped).startOf('day').toDate()` → IST local midnight (March 1 00:00 IST = Feb 28 18:30 UTC). The `ignoreTimezone=true` flag on Config B does not protect the preset path because `parseDateString` is called with hardcoded `ignoreTimezone=true` regardless of field config (Bug #3). Same behavior as Config A (5-A-IST). **Every form instance created in UTC+ timezones with date-only presets silently stores the wrong date.**
...
| Reference               | Location                                                              |
| ----------------------- | --------------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `5-B-IST`                                        |
| Summary                 | [summary](../summaries/tc-5-B-IST.md)                                 |
...
| Sibling: 5-B-BRT (PASS) | [tc-5-B-BRT.md](tc-5-B-BRT.md) — same config, no bug in BRT           |
| Sibling: 5-A-IST (FAIL) | [tc-5-A-IST.md](tc-5-A-IST.md) — Config A same Bug #7 behavior        |
