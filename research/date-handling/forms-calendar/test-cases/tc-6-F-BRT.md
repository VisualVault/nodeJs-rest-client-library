# TC-6-F-BRT — Config F, Current Date, BRT: legacy date-only + ignoreTZ correct; ignoreTZ inert on new Date() path
...
| P6  | Verify current date field exists | `VV.Form.VV.FormPartition.getValueObjectValue('Field24')` returns non-empty Date                                                                                                                                                              |

## Test Steps

| Step | Action                                                                    | Expected Result                                                       |
| ---- | ------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| 1    | Load fresh form instance                                                  | Form loads; Field24 auto-populates with today's date                  |
| 2    | Read display value of Field24                                             | Shows today's local date (04/03/2026 in BRT)                          |
| 3    | Read raw value: `VV.Form.VV.FormPartition.getValueObjectValue('Field24')` | Returns a Date object with valid ISO string                           |
| 4    | Convert raw to local date                                                 | Local date portion matches today's BRT date (04/03/2026)              |
| 5    | Read GFV: `VV.Form.getFieldValue('Field24')`                              | Returns same raw Date object                                          |
| 6    | Confirm timezone context                                                  | Raw UTC timestamp, when converted to BRT (UTC-3), yields today's date |

> **Note:** `ignoreTimezone=true` has no effect on the Current Date init path. The `new Date()` call bypasses all timezone-sensitive parsing. Behavior is identical to Config E (6-E-BRT).
...
- **Matrix row**: Category 6, Config F, BRT
- **Summary**: [tc-6-F-BRT summary](../summaries/tc-6-F-BRT.md)
- **Analysis**: [analysis.md](../../../date-handling/analysis.md)
- **Siblings**: tc-6-E-BRT, tc-6-G-BRT, tc-6-H-BRT (other BRT legacy configs)
