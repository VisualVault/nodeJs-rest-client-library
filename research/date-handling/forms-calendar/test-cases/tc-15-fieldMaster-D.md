# TC-15-fieldMaster-D — Config D fieldMaster: cross-env property comparison
...
| Target Field    | Config D — enableTime=true, ignoreTimezone=true, useLegacy=false |
...
| 2   | Locate Config D fieldMaster | `VV.Form.fieldMaster.filter(f => f.enableTime && f.ignoreTimezone && !f.useLegacy)`                                                                           | Array of Config D fields                    | ☐   |
...
**FAIL-2 (No Config D fields found):** fieldMaster filter returns empty array — verify field configuration flags.
**FAIL-3 (Unexpected mask):** v2 mask is not empty string — indicates config change since last audit.

## Related

| Reference      | Location                                                                            |
| -------------- | ----------------------------------------------------------------------------------- |
| Matrix row     | `matrix.md` — row `15-fieldMaster-D`                                                |
| WADNR run data | `projects/wadnr/testing/.../runs/audit-kendo-version-wadnr-2026-04-10.md` § Phase 2 |
| Siblings       | `tc-15-vv-core.md`, `tc-15-fieldMaster-C.md`, `tc-15-kendo-global.md`               |
