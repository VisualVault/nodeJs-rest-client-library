# TC-8B-E-BRT — Config E, GetDateObject, BRT: legacy date-only identical to Config A; correct Date
...
| 8   | Compare with GetFieldValue (DevTools console)            | `VV.Form.GetFieldValue('<FIELD_NAME>')`                        | `"2026-03-15"` — raw unchanged for legacy date-only | ☐   |
...
> `useLegacy=true` has no effect on date-only GDOC behavior. Config E produces identical results to Config A (8B-A-BRT): GDOC creates a Date at local BRT midnight, `.toISOString()` returns real UTC (+3h), and GFV returns the raw stored value unchanged.
...
- Interpretation: The function may return a string, null, or other type for legacy date-only configs. Document the actual return type and value.

**FAIL-2 (GDOC toISOString differs from expected):**
Step 7 returns a value other than `"2026-03-15T03:00:00.000Z"`.

- Interpretation: The Date object does not represent BRT midnight as expected. If legacy mode alters GDOC parsing, this would diverge from Config A behavior. Document the actual value.
...
| Reference                      | Location                                                                  |
| ------------------------------ | ------------------------------------------------------------------------- |
| Matrix row                     | `../matrix.md` — row `8B-E-BRT`                                           |
| Non-legacy identical: 8B-A-BRT | [`tc-8B-A-BRT.md`](tc-8B-A-BRT.md) — Config A GDOC in BRT (same behavior) |
| GFV comparison: 8-E            | `../matrix.md` — row `8-E` (Cat 8, Config E GFV)                          |
| Field config reference         | `research/date-handling/CLAUDE.md` — Config E                             |
