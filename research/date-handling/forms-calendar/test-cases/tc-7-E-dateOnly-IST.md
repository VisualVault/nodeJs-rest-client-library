# TC-7-E-dateOnly-IST — Config E, SetFieldValue date string, IST: stores previous day (Bug #7); useLegacy no protection
...
| **Scenario**            | `2026-03-15`, IST — legacy date-only field, expected Bug #7                              |
...
> Config E (`useLegacy=true`) does NOT protect date-only fields from Bug #7. The `useLegacy` flag only affects the `getCalendarFieldValue()` code path (GFV output), not the `normalizeCalValue()` code path (SFV storage). Date-only fields in IST store the previous day regardless of `useLegacy`.
...
- Interpretation: Bug #7 confirmed. `useLegacy=true` does NOT protect date-only fields. `normalizeCalValue()` uses `moment(input).toDate()` which parses `"2026-03-15"` as IST local midnight = `2026-03-14T18:30:00.000Z` UTC. `getSaveValue()` extracts the UTC date → stores `"2026-03-14"`.
...
| Reference              | Location                                                                               |
| ---------------------- | -------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-E-dateOnly-IST`                                                |
| Results index          | `../results.md` § Session 2026-04-03                                                   |
| Bug #7 analysis        | `../analysis.md` — Bug #7 fires in UTC+ timezones                                      |
| BRT sibling            | [`tc-7-E-dateOnly-BRT.md`](tc-7-E-dateOnly-BRT.md) — passes (BRT unaffected by Bug #7) |
| Config A IST sibling   | [`tc-7-A-dateOnly-IST.md`](tc-7-A-dateOnly-IST.md) — identical Bug #7 behavior         |
| Field config reference | `research/date-handling/CLAUDE.md` — Config E (`enableTime=false`, `useLegacy=true`)   |
