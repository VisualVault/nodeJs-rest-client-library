# TC-5-F-IST — Config F, Preset Date, IST: Bug #7 stores Feb 28; ignoreTZ + legacy does not protect preset path
...
// Expected result includes: { name: "Field20", initialDate: "2026-03-01T11:31:44.547Z" }
...
> **Note on raw type**: Before save, preset fields store a **Date object** in the value partition, not a string. The Date object's `.toLocaleDateString()` shows the correct local date, but `.toISOString()` reflects the UTC representation. For IST (UTC+5:30), local midnight March 1 = UTC Feb 28 18:30 — the UTC date is the previous day. When `getSaveValue()` extracts `toISOString().substring(0,10)`, it gets `"2026-02-28"` — permanent data corruption.
...
- Interpretation: V2 is active instead of V1. This test documents V1 behavior. V2 still falls through to `moment(stripped).toDate()` for `ignoreTimezone=true` date-only fields — Bug #7 persists in V2 for Config F.
...
- Interpretation: Bug #7 confirmed on the preset/form-init path for Config F. `initCalendarValueV1` uses hardcoded `parseDateString(initialDate, false, true)` → `moment(stripped).startOf('day').toDate()` → IST local midnight (March 1 00:00 IST = Feb 28 18:30 UTC). `ignoreTZ=true` on the field provides no protection — Bug #3 hardcodes `ignoreTimezone=true` for all presets regardless. Combined with `useLegacy=true`, neither flag prevents Bug #7 on the init path. Same behavior as non-legacy Config B (5-B-IST).

**FAIL-4 (GFV returns unexpected value):**
Step 4 returns a Date object instead of a date-only string, or returns `"2026-02-28"` instead of `"2026-03-01"`.

- Interpretation: For preset fields before save, `GetFieldValue()` returns the raw Date object (same as `getValueObjectValue()`). If it returns `"2026-02-28T18:30:00.000Z"` (the Date's ISO representation), Bug #7 is exposed through the developer API as well.
...
| Reference               | Location                                                                                |
| ----------------------- | --------------------------------------------------------------------------------------- |
| Matrix row              | `../matrix.md` — row `5-F-IST`                                                          |
| Summary                 | [summary](../summaries/tc-5-F-IST.md)                                                   |
| Bug #7 analysis         | `../analysis.md` — Bug #7 (date-only SetFieldValue wrong day in UTC+)                   |
| Bug #3 analysis         | `../analysis.md` — Bug #3 (hardcoded params in preset init path)                        |
| Sibling: 5-F-BRT (PASS) | [tc-5-F-BRT.md](tc-5-F-BRT.md) — same config, no bug in BRT                             |
| Sibling: 5-B-IST (FAIL) | [tc-5-B-IST.md](tc-5-B-IST.md) — Config B same Bug #7 behavior (non-legacy equivalent)  |
| Sibling: 5-E-IST (FAIL) | [tc-5-E-IST.md](tc-5-E-IST.md) — Config E same Bug #7 behavior (ignoreTZ=false variant) |
