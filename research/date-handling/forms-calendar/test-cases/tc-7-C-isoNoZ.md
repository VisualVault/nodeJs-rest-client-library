# TC-7-C-isoNoZ — Config C, SetFieldValue ISO without Z, BRT: treated as local; GFV returns real UTC (+3h)
...
| 4   | Capture GetFieldValue (DevTools console)         | `VV.Form.GetFieldValue('<FIELD_NAME>')`                        | `"2026-03-15T03:00:00.000Z"` — real UTC (+3h from BRT) | ☐   |
...
> ISO without Z is treated as local BRT midnight. GFV correctly converts to real UTC by adding +3h (BRT offset). No drift on round-trip because GFV returns genuine UTC — re-parsing produces the same local time.

---

## Fail Conditions

**FAIL-1 (GFV wrong UTC):**
Step 4 returns a value other than `"2026-03-15T03:00:00.000Z"`.

- Interpretation: Config C's UTC reconstruction is broken. If GFV returns `"2026-03-15T00:00:00.000Z"`, it is treating the local value as UTC (fake Z). If it returns `"2026-03-15T06:00:00.000Z"`, a double offset may be applied. Verify P6 resolved to the correct field (DataField6, not DataField5).
...
| Reference              | Location                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-isoNoZ`                                                         |
| Sibling (ISO+Z)        | [`tc-7-C-isoZ.md`](tc-7-C-isoZ.md)                                                        |
| Config D comparison    | [`tc-7-D-isoNoZ.md`](tc-7-D-isoNoZ.md) — Config D GFV adds fake Z (Bug #5)                |
