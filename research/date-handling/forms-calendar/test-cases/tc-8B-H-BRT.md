# TC-8B-H-BRT — Config H, GetDateObject, BRT: legacy DateTime correct Date; toISOString gives real UTC
...
| **Scenario**            | `2026-03-15T00:00:00`, BRT midnight — `2026-03-15T00:00:00-03:00` = `2026-03-15T03:00:00Z` UTC |
...
| 8   | Compare with GetFieldValue (DevTools console)            | `VV.Form.GetFieldValue('<FIELD_NAME>')`                        | `"2026-03-15T00:00:00"` — raw value, useLegacy prevents Bug #5 | ☐   |
...
> Config H: GDOC.toISOString() = `"2026-03-15T03:00:00.000Z"` (real UTC), while GFV = `"2026-03-15T00:00:00"` (raw local). Both are correct for their respective APIs. GDOC gives real UTC because `Date.toISOString()` always returns UTC. GFV gives raw because `useLegacy=true` bypasses the `getCalendarFieldValue()` transformation where Bug #5 lives — the legacy path returns the stored value directly without adding fake Z.
...
- Interpretation: The function may return a string, null, or other type for legacy DateTime configs. Document the actual return type and value.

**FAIL-2 (GDOC toISOString differs from expected):**
Step 7 returns a value other than `"2026-03-15T03:00:00.000Z"`.

- Interpretation: The Date object does not represent BRT midnight as expected. If legacy mode alters GDOC parsing for DateTime fields, this would be a new finding. Document the actual value.

**FAIL-3 (GFV returns fake Z):**
Step 8 returns `"2026-03-15T00:00:00.000Z"` instead of `"2026-03-15T00:00:00"`.

- Interpretation: `useLegacy=true` did not bypass Bug #5. This would contradict the established finding that legacy configs are safe from Bug #5. Document and escalate.
...
| Reference                    | Location                                                                                  |
| ---------------------------- | ----------------------------------------------------------------------------------------- |
| Matrix row                   | `../matrix.md` — row `8B-H-BRT`                                                           |
| Non-legacy sibling: 8B-D-BRT | [`tc-8B-D-BRT.md`](tc-8B-D-BRT.md) — Config D GDOC in BRT (same GDOC, but GFV has Bug #5) |
| GFV comparison: 8-H-BRT      | `../matrix.md` — row `8-H-BRT` (Cat 8, Config H GFV)                                      |
| Bug #5 analysis              | `../analysis.md` — Bug #5: useLegacy bypasses fake Z                                      |
| Field config reference       | `research/date-handling/CLAUDE.md` — Config H                                             |
