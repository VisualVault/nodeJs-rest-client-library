# TC-7-C-isoZ — Config C, SetFieldValue ISO+Z, BRT: UTC→local shift stores Mar 14 21:00; GFV returns correct UTC
...
| 4   | Capture GetFieldValue (DevTools console)      | `VV.Form.GetFieldValue('<FIELD_NAME>')`                             | `"2026-03-15T00:00:00.000Z"` — correct UTC reconstruction | ☐   |
...
> Config C (`ignoreTimezone=false`) round-trips correctly. The raw value stores local time (Mar 14 21:00 BRT = UTC midnight), and `GetFieldValue()` correctly reconstructs the UTC timestamp. This contrasts with Config D where GFV adds a fake Z (Bug #5). The day crossing to Mar 14 in the raw value is expected — it reflects UTC midnight in local BRT time.
...
- Interpretation: Config C's UTC reconstruction is broken. Expected: GFV converts local stored value back to UTC. If GFV returns `"2026-03-14T21:00:00.000Z"`, it is treating the local value as UTC (same fake Z issue as Bug #5 but on Config C).
...
| Reference              | Location                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-isoZ`                                                           |
| Analysis               | `../analysis.md` — no bugs for Config C                                                   |
| Sibling (ISO no Z)     | [`tc-7-C-isoNoZ.md`](tc-7-C-isoNoZ.md)                                                    |
| Config D comparison    | [`tc-7-D-isoZ.md`](tc-7-D-isoZ.md) — Config D GFV adds fake Z (Bug #5)                    |
