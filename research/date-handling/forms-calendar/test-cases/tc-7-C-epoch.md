# TC-7-C-epoch — Config C, SetFieldValue Epoch, BRT: BRT midnight stored as local; GFV returns real UTC
...
| 2   | Set value via epoch milliseconds (DevTools console) | `VV.Form.SetFieldValue('<FIELD_NAME>', 1773543600000)`         | Field displays `03/15/2026 12:00 AM`                      | ☐   |
...
> Epoch `1773543600000` = `2026-03-15T03:00:00.000Z` = BRT midnight. `normalizeCalValue()` converts via `moment(1773543600000).toDate()` which resolves to the same instant. Config C stores local time and GFV reconstructs real UTC. Epoch input is unambiguous — no parsing ambiguity, no timezone reinterpretation.
...
| Reference              | Location                                                                                  |
| ---------------------- | ----------------------------------------------------------------------------------------- |
| Matrix row             | `../matrix.md` — row `7-C-epoch`                                                          |
