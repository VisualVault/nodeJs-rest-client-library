# TC-6-C-IST — Config C, Current Date, IST: DateTime auto-populated with UTC; GFV returns real UTC ISO
...
| 2   | Verify field displays today with time       | Visually inspect the Current Date field (DataField17)                                                                  | Displays today's date with time in `MM/dd/yyyy hh:mm AM/PM` format (e.g., `04/04/2026 01:50 AM`) | ☐   |
| 3   | Capture raw stored value (DevTools console) | `var raw = VV.Form.VV.FormPartition.getValueObjectValue('DataField17'); raw instanceof Date ? raw.toISOString() : raw` | A UTC ISO string — Date object with today's UTC timestamp (e.g., `"2026-04-03T20:20:54.087Z"`)   | ☐   |
...
| 5   | Confirm browser timezone (DevTools console) | `new Date().toISOString()`                                                                                             | A UTC timestamp — confirms IST active when compared with local time                              | ☐   |
...
> **Note:** Config C DateTime Current Date stores a genuine UTC timestamp via `new Date()`. GFV returns the same real UTC ISO because `ignoreTimezone=false` avoids the fake Z path in `getCalendarFieldValue()`. Compare with Config D (6-D-IST) where Bug #5 corrupts GFV output.
...
| Reference                  | Location                                                                                 |
| -------------------------- | ---------------------------------------------------------------------------------------- |
| Matrix row                 | `matrix.md` — row `6-C-IST`                                                              |
| Run file                   | [run-1](../runs/tc-6-C-IST-run-1.md)                                                     |
| Summary                    | [summary](../summaries/tc-6-C-IST.md)                                                    |
| Analysis — Scenario 6      | `analysis.md` — Scenario 6: New Form with "Current Date" Default (only correct scenario) |
| Sibling: 6-C-BRT           | `summaries/tc-6-C-BRT.md` — Config C current date in BRT (PASS, real UTC)                |
| Bug #5 comparison: 6-D-IST | `summaries/tc-6-D-IST.md` — Config D current date in IST (FAIL — Bug #5)                 |
| Preset comparison: 5-C-IST | `matrix.md` — row `5-C-IST`                                                              |
