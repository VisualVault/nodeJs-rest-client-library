...
async function main() {
    ...
    if (dataRoot && Array.isArray(dataRoot)) {
        ...
        if (items.length > 0) {
            ...
            const scriptFields = keys.filter((k) => /script|code|body|source|content|text|js|function/i.test(k));
            console.log(
                `Fields matching script/code/body/source: ${scriptFields.length > 0 ? scriptFields.join(', ') : 'NONE'}`
            );
